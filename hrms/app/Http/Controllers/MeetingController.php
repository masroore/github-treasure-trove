...
class MeetingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('Create Meeting')) {
            ...
            if (isset($setting['meeting_notification']) && 1 == $setting['meeting_notification']) {
            ...
            $setting = Utility::settings(\Auth::user()->creatorId());
            $branch = Branch::find($request->branch_id);
            if (isset($setting['telegram_meeting_notification']) && 1 == $setting['telegram_meeting_notification']) {
                $msg = $request->title . ' ' . __('meeting created for') . ' ' . $branch->name . ' ' . ('from') . ' ' . $request->date . ' ' . ('at') . ' ' . $request->time . '.';
