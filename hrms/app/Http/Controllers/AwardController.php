...
class AwardController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Award')) {
            ...
            if (isset($setting['award_notificaation']) && 1 == $setting['award_notificaation']) {
            ...
            $awardtype = AwardType::find($request->award_type);
            $emp = Employee::find($request->employee_id);
            if (isset($setting['telegram_award_notification']) && 1 == $setting['telegram_award_notification']) {
                $msg = $awardtype->name . ' ' . __('created for') . ' ' . $emp->name . ' ' . __('from') . ' ' . $request->date . '.';
            ...
            $awardtype = AwardType::find($request->award_type);
            $emp = Employee::find($request->employee_id);
            if (isset($setting['twilio_award_notification']) && 1 == $setting['twilio_award_notification']) {
                $msg = $awardtype->name . ' ' . __('created for') . ' ' . $emp->name . ' ' . __('from') . ' ' . $request->date . '.';
                Utility::send_twilio_msg($emp->phone, $msg);
            }

            $setings = Utility::settings();
            if (1 == $setings['award_create']) {
                ...
                return redirect()->route('award.index')->with('success', __('Award  successfully created.') . ($smtp_error ?? ''));
            }
