...
class HolidayController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Holiday')) {
            ...
            if (isset($setting['Holiday_notification']) && 1 == $setting['Holiday_notification']) {
            ...
            // telegram
            $setting = Utility::settings(\Auth::user()->creatorId());
            if (isset($setting['telegram_Holiday_notification']) && 1 == $setting['telegram_Holiday_notification']) {
                $msg = $request->occasion . ' ' . __('on') . ' ' . $request->date . '.';
