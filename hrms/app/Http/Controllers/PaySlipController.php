...
class PaySlipController extends Controller
{
    ...
    {
        if (\Auth::user()->can('Manage Pay Slip') || 'employee' == \Auth::user()->type) {
    ...
    public function store(Request $request)
    {
        ...
        if ($payslip_employee > \count($validatePaysilp)) {
            ...
            foreach ($employees as $employee) {
                ...
                if (isset($setting['monthly_payslip_notification']) && 1 == $setting['monthly_payslip_notification']) {
                ...
                $setting = Utility::settings(\Auth::user()->creatorId());
                $month = date('M Y', strtotime($payslipEmployee->salary_month . ' ' . $payslipEmployee->time));
                if (isset($setting['telegram_monthly_payslip_notification']) && 1 == $setting['telegram_monthly_payslip_notification']) {
                    $msg = ('payslip generated of') . ' ' . $month . '.';
                ...
                if (isset($setting['twilio_payslip_notification']) && 1 == $setting['twilio_payslip_notification']) {
    ...
    public function send($id, $month)
    {
        ...
        if (1 == $setings['payroll_create']) {
            ...
            return redirect()->back()->with('success', __('Payslip successfully sent.') . ($smtp_error ?? ''));
        }
