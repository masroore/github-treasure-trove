...
class AnnouncementController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Announcement')) {
            ...
            if (isset($setting['Announcement_notification']) && 1 == $setting['Announcement_notification']) {
            ...
            $setting = Utility::settings(\Auth::user()->creatorId());
            $branch = Branch::find($request->branch_id);
            if (isset($setting['telegram_Announcement_notification']) && 1 == $setting['telegram_Announcement_notification']) {
                $msg = $request->title . ' ' . __('announcement created for branch') . ' ' . $branch->name . ' ' . __('from') . ' ' . $request->start_date . ' ' . __('to') . ' ' . $request->end_date . '.';
            ...
            if (isset($setting['twilio_announcement_notification']) && 1 == $setting['twilio_announcement_notification']) {
