          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tr>
              <td class="text-center">
                <input type="checkbox" class="checkboxes" name="ID" onclick="clicked();" value="{{ $admin->id }}" />
              </td>
              <td class="text-center">{{$admin->name}}</td>
              <td class="text-center">
                <a href="{{ $admin->avatar_url }}" class="fancybox-button" data-rel="fancybox-button">
                  <img class="img-responsive img-circle" src="{{ correctProImgPath($admin->avatar_url) }}" onerror="noExistProImg(this)" alt="" style="width:50px;display:inline-block;">
                </a>
              </td>
              <td class="text-center">{{$admin->email}}</td>
              <td class="text-center">
                <div class="btn-group-xs">
                <a href="{{ url('adminDetail/') }}/{{$admin->id}}" class="btn_detail admin_detail">
                    Detail
                </a>
                <input type="hidden" value="{{ $admin->id }}">
                <input type="hidden" value="{{ $admin->name }}">
                </div>
              </td>
            </tr>
            @endforeach
  ...
  <div class="modal-dialog">
    <div class="modal-content bg-primary">
      ...
      <div class="modal-footer justify-content-between">
        ...
        <button type="button" class="btn btn-outline-light" onclick="deleteAdmins()">Delete</button>
      </div>
    </div>
    <!-- /.modal-content -->
  </div>
...
<script>
  ...
  function deletAdmins() {
    ...
    $.get('{{url('adminDel')}}' + "/" + IDs, function(data) {
      location.href = '';
    })
  }
