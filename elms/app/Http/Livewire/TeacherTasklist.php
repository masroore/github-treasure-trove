...
class TeacherTasklist extends Component
{
    ...
    public function mount(Task $task): void
    {
        ...
        if (session('task' . $task->id . 'filter')) {
            $this->submissionFilter = session('task' . $task->id . 'filter');
        } else {
    ...
    public function render()
    {
        ...
        if ($this->search) {
            $query = $query->whereHas('user', function ($q): void {
    ...
    public function updatingSubmissionFilter($value): void
    {
