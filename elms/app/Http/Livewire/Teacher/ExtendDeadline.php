...
class ExtendDeadline extends Component
{
    ...
    public function render()
    {
        if ($this->email) {
            $this->showQuery = true;
            $this->students = User::where('name', 'like', "%$this->email%")->whereHas('roles', function (Builder $query): void {
    ...
    public function addStudent()
    {
        ...
        if (!$u) {
            return session()->flash('error', 'No student found with that email.');
        }
        if ($u->cannot('view', $this->task)) {
            return session()->flash('error', 'Student has no access to this task.');
        }
    ...
    public function extendDeadline()
    {
        ...
        switch ($this->scope) {
            case 'all':
                ...
                break;
            case 'selected':
                if (!$this->selected_students->count()) {
                    return session()->flash('error', 'You need to add at least one student.');
                }
