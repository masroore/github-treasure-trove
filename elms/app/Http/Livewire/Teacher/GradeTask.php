...
class GradeTask extends Component
{
    ...
    public function mount(Task $task): void
    {
        ...
        if ($this->submission) {
            $this->submission = $this->submission->pivot;
        } else {
            abort(404);
        }
        ...
        if ($this->submission->assessment) {
            $this->items = json_decode($this->submission->assessment, true);
        }
    }
    ...
    public function showEssayGrader($key): void
    {
    ...
    public function gradeEssay(): void
    {
    ...
    public function getCorrect(): void
    {
        foreach ($this->task_content as $key => $content) {
            if (array_key_exists('answer', $content)) {
                if (strcasecmp(sanitizeString($this->answers[$key]['answer']), sanitizeString($content['answer'])) == 0) {
                    $this->items[] = $key = ['isCorrect' => true, 'score' => $content['points']];
                } else {
                ...
                foreach ($studentEnums as $key => $enumItem) {
                    if (in_array(sanitizeString($enumItem), $correctEnums)) {
                        ++$correctItems;
                    }
                }
                if (count($correctEnums) == $correctItems) {
                    $this->items[] = $key = ['isCorrect' => true, 'score' => $content['points'] * count($correctEnums)];
                } elseif ($correctItems) {
                    $this->items[] = $key = ['isCorrect' => 'partial', 'score' => $content['points'] * $correctItems];
                } else {
                    $this->items[] = $key = ['isCorrect' => false, 'score' => 0];
                }
            } else {
    ...
    public function markAsCorrect($key): void
    {
    ...
    public function markAsWrong($key): void
    {
        $this->items[$key] = ['isCorrect' => false, 'score' => 0];
    }

    public function partialPoints($key): void
    {
        $this->validate([
