...
class Chat extends Component
{
    ...
    public function newMessageEvent(): void
    {
    ...
    public function changeChatroom(Chatroom $chatroom): void
    {
    ...
    {
        $this->validate();
        if (!$this->current_chatroom && !$this->newMessage) {
            return;
        }
        if ($this->newMessage) {
        ...
        $this->messageContent = '';
        if ($this->current_chatroom->isGroup) {
            event(new SendMessage($this->current_chatroom->members, auth()->id()));
        } else {
            broadcast(new NewMessage($this->current_chatroom->receiver));
        }
    ...
    public function sendToNewContact($contact)
    {
        if ($contact['id'] == auth()->id()) {
            return $this->alert('error', 'You cannot send to yourself.');
        }
        $chatroom = auth()->user()->chatrooms->where('isGroup', false)->where('receiver.id', $contact['id'])->first();
        if ($chatroom) {
            return $this->current_chatroom = $chatroom;
        }
