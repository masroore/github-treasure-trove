...
class WorkloadUploader extends Component
{
    ...
    public function uploadWorkload()
    {
        ...
        $handle = fopen(storage_path("app/$workloadpath"), 'rb');
        while (($data = fgetcsv($handle)) !== false) {
            $this->workloadArray[] = $data;
        }
        array_splice($this->workloadArray, 0, 5);
        foreach ($this->workloadArray as $key => $workload) {
            $this->workloadArray[$key] = array_values(array_filter($workload, fn ($value) => null !== $value && $value !== ''));
        }
        $this->workloadArray = array_filter($this->workloadArray, fn ($value) => null !== $value && $value !== '' && $value !== []);
        ...
        DB::transaction(function (): void {
            ...
            if ($workloadChanged) {
                $this->teacher->user->notify(new GeneralNotification('Your workload has been updated.', route('teacher.faculty_workload')));
            }
        });
        $this->workloadArray = [];
        ++$this->fileId;
    ...
    public function removeSection(Section $section, $teacher_id): void
    {
