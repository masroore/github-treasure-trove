...
class TaskMaker extends Component
{
    ...
    public function mount(): void
    {
        ...
        if (request('draft_id')) {
            ...
            if ($draft?->teacher_id != $teacher->id) {
                abort(403);
            }
        } else {
            if (!request('module') || !request('course') || !request('type')) {
                abort(404);
            }
            if (!TaskType::whereName(request('type'))->first()) {
                abort(403);
            }
    ...
    public function rubricSet($rubric): void
    {
        $this->task_rubric = $rubric;
        $this->isRubricSet = true;
    }

    public function hideRubricEdit(): void
    {
        $this->showrubric = false;
    }

    public function ExpectAttachment($key): void
    {
        $this->items[$key]['attachment'] = !$this->items[$key]['attachment'];
        if ($this->items[$key]['attachment']) {
            $this->alert('success', 'Question ' . ($key + 1) . ' will require file attachment.', ['position' => 'center', 'timer' => 1000]);
        } else {
            $this->alert('success', 'Question ' . ($key + 1) . ' will not require file attachment.', ['position' => 'center', 'timer' => 1000]);
        }
    }

    public function TorFtrigger($key): void
    {
    ...
    public function enumerationTrigger($key): void
    {
    ...
    public function removeEnumItem($key, $enum): void
    {
        array_splice($this->items[$key]['enumerationItems'], $enum, 1);
    }

    public function addEnumerationItem($key): void
    {
        $this->items[$key]['enumerationItems'][] = '';
    }

    public function Essaytrigger($key): void
    {
    ...
    public function updated($name, $value): void
    {
        if (preg_match('/items.\d+.points/', $name)) {
            if (!is_numeric($value)) {
                $this->items[(int) (explode('.', $name)[1])]['points'] = 1;
            }
        }
    }
    ...
    public function addItem($key): void
    {
    ...
    public function addOption($key): void
    {
    ...
    public function removeOption($key, $id): void
    {
    ...
    public function saveTask()
    {
        ...
        if (!$this->noDeadline && Carbon::parse($this->date_due . ' ' . $this->time_due) < now()->addHour()) {
            return session()->flash('error', 'Task deadline must at least be one hour from now.');
        }
        if (!$this->openImmediately) {
            ...
            if (Carbon::now()->addMinutes(30) > Carbon::parse($this->date_open . ' ' . $this->time_open)) {
                return session()->flash('error', 'Task opening must at least be 30 minutes later than the current time.');
            }
            $due = Carbon::parse($this->date_due . ' ' . $this->time_due);
            $open = Carbon::parse($this->date_open . ' ' . $this->time_open);
            if ($due->isBefore($open)) {
                return session()->flash('error', 'Cannot set the deadline before task opens.');
            }
        }
        ...
        foreach ($this->items as $key => $item) {
            if (!$item['enumeration'] && !isset($item['answer'])) {
        ...
        DB::transaction(function (): void {
            if (!$this->task_instructions) {
                $this->task_instructions = null;
            }
            if (!$this->noDeadline) {
            ...
            if (!$this->openImmediately) {
                $open_on = $this->date_open . ' ' . $this->time_open;
            } else {
                $open_on = null;
            }
            if (count($this->matchingTypeOptions)) {
                $matchingtype_options = json_encode($this->matchingTypeOptions);
            } else {
                $matchingtype_options = null;
            }
            if ($this->allSection) {
                $sections = $this->course->sections()->where('teacher_id', auth()->user()->teacher->id)->get();
    ...
    public function resetRubric(): void
    {
        $this->rubric = [
            'criteria' => [
                [
    ...
    public function addPerformanceRating()
    {
        ...
        $this->rubric['performance_rating'][] = $this->new_performance_rating;
        $this->new_performance_rating = '';
    }

    public function editCriterionName($id): void
    {
    ...
    public function editCriterionWeight($id): void
    {
    ...
    public function equalWeights(): void
    {
        $criterion_count = count($this->rubric['criteria']);
        foreach ($this->rubric['criteria'] as $key => $criterion) {
            $this->rubric['criteria'][$key]['weight'] = round(100 / $criterion_count, 2);
    ...
    public function addMatchingTypeOption(): void
    {
        $this->validate([
            'newMatchingTypeOption' => 'required',
        ]);
        $this->matchingTypeOptions[] = $this->newMatchingTypeOption;
        $this->newMatchingTypeOption = '';
    }

    public function removeMatchingTypeOption($g): void
    {
