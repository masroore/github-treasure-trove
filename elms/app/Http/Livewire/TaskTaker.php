...
class TaskTaker extends Component
{
    ...
    public function filenameTooLong(): void
    {
    ...
    public function mount(Task $task): void
    {
        ...
        $this->task = $task;
        if ($task->matchingtype_options) {
            $this->matchingTypeOptions = json_decode($task->matchingtype_options, true);
        }
        ...
        foreach ($this->task_content as $key => $content) {
            ...
            if ($content['enumeration']) {
                $this->enumeration[$key] = ['item_no' => $content['item_no'], 'items' => []];
            }
        }
    }
    ...
    {
        if (auth()->user()->cannot('submit', $this->task)) {
            return session()->flash('deadline', 'Unfortunately, this task has already been closed.');
        }
        ...
        foreach ($ta as $key => $item) {
            if (isset($item['files']) && $item['files'] != []) {
                if (sanitizeString($item['answer']) == '') {
                    $ta[$key]['answer'] = 'File attached.';
                }
            }
        }
