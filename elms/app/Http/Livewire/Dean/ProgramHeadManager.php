...
class ProgramHeadManager extends Component
{
    ...
    public function render()
    {
        if ($this->email) {
            $this->showQuery = true;
            $this->teachers = User::where('name', 'like', "%$this->email%")->whereHas('roles', function (Builder $query): void {
        ...
        if ($this->teachers->count() == 1 && $this->teachers->first()->email == $this->email) {
            $this->showQuery = false;
        }
    ...
    public function removeProgramHead(Department $department, ProgramHead $programhead): void
    {
