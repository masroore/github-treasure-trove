...
class PreviewSubmission extends Component
{
    ...
    public function mount(StudentTask $submission): void
    {
        $this->submission = $submission;
        $this->task = $submission->task;
        if ($this->task->matchingtype_options) {
            $this->matchingTypeOptions = json_decode($this->task->matchingtype_options, true);
        }
