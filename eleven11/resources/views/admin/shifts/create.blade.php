        ...
        <form method="POST" action="{{ route("admin.shifts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('days') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="days" id="days" value="1" required {{ old('days', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('frequency') ? 'is-invalid' : '' }}" name="frequency" id="frequency">
                    ...
                    @foreach(App\Models\Shift::FREQUENCY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('frequency', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
