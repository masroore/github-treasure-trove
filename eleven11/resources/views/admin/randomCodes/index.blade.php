...
@can('random_code_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.random-codes.create') }}">
                        ...
                        <tr data-entry-id="{{ $randomCode->id }}">
                            ...
                            <td>
                                @can('random_code_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.random-codes.show', $randomCode->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('random_code_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.random-codes.edit', $randomCode->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('random_code_delete')
                                    <form action="{{ route('admin.random-codes.destroy', $randomCode->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('random_code_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.random-codes.massDestroy') }}",
