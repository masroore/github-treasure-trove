...
class ReportController extends Controller
{
    ...
    public function loan_transaction(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            $data = LoanRepayment::where('branch_id', session('branch_id'))->whereBetween(
                'collection_date',
                [$start_date, $end_date]
            )->get();
        } else {
    ...
    public function trial_balance_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            Excel::create(
                ...
                function ($excel) use ($data): void {
                    $excel->sheet('Sheet', function ($sheet) use ($data): void {
    ...
    public function expected_repayments_excel(Request $request)
    {
        ...
        $data = [];
        array_push($data, [
            trans_choice('general.expected', 1) . ' ' . trans_choice(
                'general.repayment',
                2
            ),
        ]);
        ...
        array_push($data, [
            ...
            number_format(
                $due_items['principal'] + $due_items['interest'] + $due_items['fees'] + $due_items['penalty'],
                2
            ),
        ]);
        array_push($data, [
            ...
            number_format(
                $paid_items['principal'] + $paid_items['interest'] + $paid_items['fees'] + $paid_items['penalty'],
                2
            ),
        ]);
        array_push($data, [
            ...
            number_format(
                ($due_items['principal'] + $due_items['interest'] + $due_items['fees'] + $due_items['penalty']) - ($paid_items['principal'] + $paid_items['interest'] + $paid_items['fees'] + $paid_items['penalty']),
                2
            ),
        ]);
    ...
    public function repayments_report_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            $data = [];
            array_push($data, [
                trans_choice('general.repayment', 1) . ' ' . trans_choice(
                    'general.report',
                    1
                ),
            ]);
            ...
            foreach (LoanTransaction::where(
                ...
                $principal = JournalEntry::where('loan_transaction_id', $key->id)->where(
                    'reversed',
                    0
                )->where('transaction_sub_type', 'repayment_principal')->sum('credit');
                $interest = JournalEntry::where('loan_transaction_id', $key->id)->where(
                    'reversed',
                    0
                )->where('transaction_sub_type', 'repayment_interest')->sum('credit');
                $fees = JournalEntry::where('loan_transaction_id', $key->id)->where(
                    'reversed',
                    0
                )->where('transaction_sub_type', 'repayment_fees')->sum('credit');
                $penalty = JournalEntry::where('loan_transaction_id', $key->id)->where(
                    'reversed',
                    0
                )->where('transaction_sub_type', 'repayment_penalty')->sum('credit');
            ...
            Excel::create(
                trans_choice('general.repayment', 2) . ' ' . trans_choice(
                'general.report',
                2
            ),
                function ($excel) use ($data): void {
    ...
    public function collection_sheet(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            if ($request->user_id == '-1') {
                $data = Loan::where('status', 'disbursed')->where(
                    'branch_id',
                    session('branch_id')
                )->get();
            } else {
                $data = Loan::where('loan_officer_id', $request->user_id)->where(
                    'branch_id',
                    session('branch_id')
    ...
    public function collection_sheet_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            if ($request->user_id == '-1') {
                $ldata = Loan::where('status', 'disbursed')->where(
                    'branch_id',
                    session('branch_id')
                )->get();
            } else {
                $ldata = Loan::where('loan_officer_id', $request->user_id)->where(
                    'branch_id',
                    session('branch_id')
                )->where('status', 'disbursed')->get();
            }
            $data = [];
            array_push($data, [
                trans_choice('general.collection', 1) . ' ' . trans_choice(
                    'general.sheet',
                    2
                ),
            ]);
            array_push($data, [
                ...
                trans_choice('general.maturity', 1) . ' ' . trans_choice(
                    'general.date',
                    1
                ),
                trans_choice('general.expected', 1) . ' ' . trans_choice(
            ...
            foreach ($ldata as $key) {
                $schedule = \App\Models\LoanSchedule::where('loan_id', $key->id)->whereBetween(
                    'due_date',
                    [$start_date, $end_date]
                )->orderBy('due_date', 'desc')->limit(1)->first();
                if (!empty($schedule)) {
                    $balance = \App\Helpers\GeneralHelper::loan_total_balance($key->id);
                    $loan_due_items = \App\Helpers\GeneralHelper::loan_due_items(
                        $key->id,
                        $key->release_date,
                        $schedule->due_date
                    );
                    $loan_paid_items = \App\Helpers\GeneralHelper::loan_paid_items(
                        $key->id,
                        $key->release_date,
                        $schedule->due_date
                    );
    ...
    public function arrears_report_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            ...
            array_push($data, [
                ...
                trans_choice('general.outstanding', 1),
                trans_choice('general.due', 1),
                trans_choice('general.day', 2) . ' ' . trans_choice(
                    'general.in',
                    1
                ) . ' ' . trans_choice('general.arrears', 2),
                trans_choice('general.day', 2) . ' ' . trans_choice(
                    'general.since',
                    1
                ) . ' ' . trans_choice(
            ...
            $total_penalty = 0;
            $total_amount = 0;
            foreach (Loan::where('first_payment_date', '<=', $end_date)->where(
                'branch_id',
                session('branch_id')
            )->where('status', 'disbursed')->orderBy('release_date', 'asc')->get() as $key) {
                $loan_due_items = GeneralHelper::loan_due_items(
                    $key->id,
                    $key->release_date,
                    $end_date
                );
                $loan_paid_items = GeneralHelper::loan_paid_items(
                    $key->id,
                    $key->release_date,
                    $end_date
                );
                ...
                if ($due > 0) {
                    ...
                    $schedules = LoanSchedule::where('loan_id', $key->id)->where(
                        'due_date',
                        '<=',
                        $end_date
                    ...
                    $transaction = LoanTransaction::where(
                        'loan_id',
                        $key->id
                    )->where(
                            'transaction_type',
                            'repayment'
                        )->where('reversed', 0)->orderBy('date', 'desc')->first();
            ...
            Excel::create(
                trans_choice('general.arrears', 1) . ' ' . trans_choice(
                'general.report',
                2
            ),
                function ($excel) use ($data): void {
    ...
    public function disbursed_loans(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            if ($request->loan_product_id == '-1') {
                ...
                $data = Loan::where('loan_product_id', $request->loan_product_id)->where(
                    'branch_id',
                    session('branch_id')
                )->where('status', 'disbursed')->whereBetween(
                        'release_date',
                        [$start_date, $end_date]
                    )->get();
            }
    ...
    public function disbursed_loans_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            if ($request->loan_product_id == '-1') {
                ...
                $ldata = Loan::where('loan_product_id', $request->loan_product_id)->where(
                    'branch_id',
                    session('branch_id')
                )->where('status', 'disbursed')->whereBetween(
                        'release_date',
            ...
            $data = [];
            array_push($data, [
                trans_choice('general.disbursed', 1) . ' ' . trans_choice(
                    'general.loan',
                    2
                ),
            ]);
    ...
    public function borrower_numbers(Request $request)
    {
        ...
        return view(
            'borrower_report.borrower_numbers',
            compact(
                'start_date',
                'end_date',
                'data'
    ...
    public function borrower_numbers_pdf(Request $request)
    {
        ...
        if (!empty($end_date)) {
            $pdf = PDF::loadView('borrower_report.borrower_numbers_pdf', compact(
                'start_date',
                'end_date',
                'data'
            ));
            //$pdf->setPaper('A4', 'landscape');
            return $pdf->download(trans_choice('general.borrower', 1) . ' ' . trans_choice(
                'general.number',
                2
            ) . '.pdf');
        }
    ...
    public function borrower_numbers_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            $data = [];
            array_push($data, [
                trans_choice('general.borrower', 1) . ' ' . trans_choice(
                    'general.number',
                    2
                ),
            ]);
            ...
            foreach (Borrower::all() as $key) {
                ...
                if ($start_date <= date_format(
                    date_create($key->created_at),
                    'Y-m-d '
                ) && $end_date >= date_format(date_create($key->created_at), 'Y-m-d ')
                ) {
    ...
    public function provisioning_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            $data = [];
            array_push($data, [
                trans_choice('general.provisioning', 1) . ' ' . trans_choice(
                    'general.report',
                    2
                ),
            ]);
            ...
            array_push($data, [
                trans_choice('general.loan_officer', 1),
                trans_choice('general.borrower', 1),
                trans_choice('general.loan', 1) . ' ' . trans_choice(
                    'general.id',
                    1
                ),
                trans_choice('general.product', 1),
            ...
            $total_amount = 0;
            foreach (Loan::where('release_date', '<=', $end_date)->where(
                'branch_id',
                session('branch_id')
            )->where('status', 'disbursed')->orderBy('release_date', 'asc')->get() as $key) {
                $loan_due_items = \App\Helpers\GeneralHelper::loan_due_items($key->id);
                $loan_due_items_arrears = \App\Helpers\GeneralHelper::loan_due_items(
                    $key->id,
                    $key->release_date,
                    $end_date
                );
    ...
    public function products_summary_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            $data = [];
            array_push($data, [
                trans_choice('general.product', 2) . ' ' . trans_choice(
                    'general.summary',
                    1
                ),
            ]);
            ...
            foreach (LoanProduct::get() as $key) {
                ...
                foreach (Loan::where('loan_product_id', $key->id)->where(
                    'branch_id',
                    session('branch_id')
                )->whereIn(
                        'status',
    ...
    public function general_report(Request $request)
    {
        ...
        for ($i = 1; $i < 14; ++$i) {
            ...
            foreach (ChartOfAccount::where('account_type', 'income')->get() as $key) {
                $cr = JournalEntry::where('account_id', $key->id)->where(
                    'year',
                    $d[0]
                )->where(
                        'month',
                ...
                $dr = JournalEntry::where('account_id', $key->id)->where(
                    'year',
                    $d[0]
                )->where(
                        'month',
    ...
    public function savings_transactions(Request $request)
    {
        ...
        $end_date = $request->end_date;
        if (!empty($start_date)) {
            $data = SavingTransaction::where('reversed', 0)->where(
                'branch_id',
                session('branch_id')
            )->whereBetween(
                    'date',
    ...
    public function savings_transactions_pdf(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            return $pdf->download(trans_choice('general.saving', 2) . ' ' . trans_choice(
                'general.transaction',
                2
            ) . '.pdf');
        }
    ...
    public function savings_transactions_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            $data = [];
            array_push($data, [
                trans_choice('general.saving', 2) . ' ' . trans_choice(
                    'general.transaction',
                    2
                ),
            ]);
            ...
            $cr = 0;
            $dr = 0;
            foreach (SavingTransaction::where('reversed', 0)->where(
                'branch_id',
                session('branch_id')
            )->whereBetween(
                    'date',
