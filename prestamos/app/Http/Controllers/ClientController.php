...
class ClientController extends Controller
{
    ...
    {
        $custom_fields = CustomFieldMeta::where('category', 'savings')->where(
            'parent_id',
            $saving->id
        )->get();
        $pdf = PDF::loadView(
    ...
    public function paySaving(Request $request, $saving)
    {
        ...
        if (Setting::where(
            'setting_key',
            'paypal_enabled'
        )->first()->setting_value == 1 && !empty(Setting::where(
                    'setting_key',
                    'paypal_email'
                )->first()->setting_value)
        ) {
        ...
        if (Setting::where(
            ...
            'paynow_enabled'
                    ...
                    'setting_key',
                    'paynow_key'
                )->first()->setting_value) && !empty(Setting::where(
                    'setting_key',
                    'paynow_id'
                )->first()->setting_value)
        ) {
    ...
    public function paynowResultSaving(Request $request, $saving): void
    {
        $status = $request->status;
        if ($status == 'Paid' || $status = 'Awaiting Delivery' || $status = 'Delivered') {
    ...
    public function stripeSaving(Request $request, $saving): void
    {
        $stripe = [
    ...
    public function paypalIPNSaving(Request $request): void
    {
...
// PAYMENT INVALID & INVESTIGATE MANUALY!
    ...
    public function paynowResult(Request $request, $loan): void
    {
        $status = $request->status;
        if ($status == 'Paid' || $status = 'Awaiting Delivery' || $status = 'Delivered') {
            //payment successful update database and show download
    ...
    //stripe payments
    public function stripe(Request $request, $loan): void
    {
        $stripe = [
            'secret_key' => Setting::where('setting_key', 'stripe_secret_key')->first()->setting_value,
            'publishable_key' => Setting::where('setting_key', 'stripe_publishable_key')->first()->setting_value,
    ...
    public function paypalIPN(Request $request): void
    {
        // read the post from PayPal system and add 'cmd'
        $req = 'cmd=_notify-validate';
        foreach ($_POST as $key => $value) {
...
// PAYMENT INVALID & INVESTIGATE MANUALY!
    ...
    public function mpesa(Request $request, $loan)
    {
        $url = Setting::where(
            'setting_key',
            'mpesa_endpoint'
        )->first()->setting_value . '/oauth/v1/generate?grant_type=client_credentials';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        $credentials = base64_encode(Setting::where(
            'setting_key',
            'mpesa_consumer_key'
        )->first()->setting_value . ':' . Setting::where(
                    'setting_key',
                    'mpesa_consumer_secret'
                )->first()->setting_value);
        curl_setopt(
    ...
    public function validate_mpesa_loan(): void
    {
        try {
    ...
    public function confirm_mpesa_loan(): void
    {
        try {
            //Set the response content type to application/json
            header('Content-Type:application/json');
    ...
    public function validate_mpesa_saving(): void
    {
        try {
            //Set the response content type to application/json
            header('Content-Type:application/json');
    ...
    public function confirm_mpesa_saving(): void
    {
        try {
            //Set the response content type to application/json
            header('Content-Type:application/json');
