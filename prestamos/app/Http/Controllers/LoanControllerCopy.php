...
class LoanControllerCopy extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach (LoanFee::all() as $key) {
            if (!empty(LoanFeeMeta::where('loan_fees_id', $key->id)->where('parent_id', $id)->where(
                'category',
                'loan'
            )->first())
            ) {
                $loan_fee = LoanFeeMeta::where('loan_fees_id', $key->id)->where('parent_id', $id)->where(
                    'category',
                    'loan'
                )->first();
            } else {
    ...
    public function createBulkRepayment()
    {
        ...
        foreach (Loan::all() as $key) {
            $loans[$key->id] = $key->borrower->first_name . ' ' . $key->borrower->last_name . '(' . trans_choice(
                'general.loan',
                1
            ) . '#' . $key->id . ',' . trans_choice(
                        'general.due',
                ...
                if (!empty($loan->loan_product)) {
                    if ($loan->loan_product->enable_late_repayment_penalty == 1) {
                        ...
                        foreach ($schedules as $schedule) {
                            if (GeneralHelper::loan_total_due_period(
                                $loan->id,
                                $schedule->due_date
    ...
    public function storeRepayment(Request $request, $loan)
    {
        ...
        $repayment->month = $date[1];
        //determine which schedule due date the payment applies too
        $schedule = LoanSchedule::where('due_date', '>=', $request->collection_date)->where(
            'loan_id',
            $loan->id
        )->orderBy(
                    'due_date',
            ...
            if ($request->notify_method == 'both') {
                ...
                if (!empty($borrower->email)) {
                    ...
                    $body = str_replace(
                        '{loanBalance}',
                        round(
                                GeneralHelper::loan_total_due_amount($loan->id) - GeneralHelper::loan_total_paid($loan->id),
                                2
                            ),
                        $body
                    );
                    ...
                    $body = str_replace(
                        '{loansBalance}',
                        round(
                                (GeneralHelper::borrower_loans_total_due($borrower->id) - GeneralHelper::borrower_loans_total_paid($borrower->id)),
                                2
                            ),
                        $body
                    );
                    ...
                    Mail::raw($body, function ($message) use ($loan , $borrower, $file_name): void {
                        $message->from(
                        ...
                        $message->attach(
                            public_path() . '/uploads/temporary/repayment_receipt' . $loan->id . '.pdf',
                            ['as' => $file_name]
                        );
                    ...
                    if (Setting::where('setting_key', 'sms_enabled')->first()->setting_value == 1) {
                        ...
                        if (!empty($borrower->mobile)) {
                            ...
                            if ($active_sms == 'routesms') {
                                ...
                                $username = Setting::where(
                                    'setting_key',
                                    'routesms_username'
                                )->first()->setting_value;
                                $password = Setting::where(
                                    'setting_key',
                                    'routesms_password'
                                )->first()->setting_value;
                            ...
                            if ($active_sms == 'clickatell') {
                                ...
                                $response = $clickatell->sendMessage([$borrower->mobile], $body);
                            }
...
//    print repayment
    public function pdfRepayment($loan, $repayment): void
    {
        PDF::AddPage();
        PDF::writeHTML(View::make('loan_repayment.pdf', compact('loan', 'repayment'))->render());
        PDF::SetAuthor('Tererai Mugova');
        PDF::Output(
            $loan->borrower->title . ' ' . $loan->borrower->first_name . ' ' . $loan->borrower->last_name . ' - Repayment Receipt.pdf',
            'D'
        );
    }
    ...
    public function pdfSchedule($loan): void
    {
        $schedules = LoanSchedule::where('loan_id', $loan->id)->orderBy('due_date', 'asc')->get();
        PDF::AddPage();
        ...
        PDF::SetAuthor('Tererai Mugova');
        PDF::Output(
            $loan->borrower->title . ' ' . $loan->borrower->first_name . ' ' . $loan->borrower->last_name . ' - Loan Repayment Schedule.pdf',
            'D'
        );
    }
    ...
    public function pdfLoanStatement($loan): void
    {
        $payments = LoanRepayment::where('loan_id', $loan->id)->orderBy('collection_date', 'asc')->get();
        PDF::AddPage();
        ...
        PDF::SetAuthor('Tererai Mugova');
        PDF::Output(
            $loan->borrower->title . ' ' . $loan->borrower->first_name . ' ' . $loan->borrower->last_name . ' - Loan Statement.pdf',
            'D'
        );
    }
    ...
    public function pdfBorrowerStatement($borrower): void
    {
        ...
        PDF::SetAuthor('Tererai Mugova');
        PDF::Output(
            $borrower->title . ' ' . $borrower->first_name . ' ' . $borrower->last_name . ' - Client Statement.pdf',
            'D'
        );
    }
    ...
    public function emailBorrowerStatement($borrower)
    {
        if (!empty($borrower->email)) {
            ...
            Mail::raw($body, function ($message) use ($borrower, $file_name): void {
                $message->from(
                    Setting::where('setting_key', 'company_email')->first()->setting_value,
                    Setting::where('setting_key', 'company_name')->first()->setting_value
                );
                ...
                $headers = $message->getHeaders();
                $message->attach(
                    public_path() . '/uploads/temporary/borrower_statement' . $borrower->id . '.pdf',
                    ['as' => $file_name]
                );
                $message->setContentType('text/html');
                $message->setSubject(Setting::where(
    ...
    public function emailLoanStatement($loan)
    {
        ...
        if (!empty($borrower->email)) {
            ...
            Mail::raw($body, function ($message) use ($borrower, $loan, $file_name): void {
                $message->from(
                    Setting::where('setting_key', 'company_email')->first()->setting_value,
                    Setting::where('setting_key', 'company_name')->first()->setting_value
                );
                ...
                $headers = $message->getHeaders();
                $message->attach(
                    public_path() . '/uploads/temporary/loan_statement' . $loan->id . '.pdf',
                    ['as' => $file_name]
                );
                $message->setContentType('text/html');
                $message->setSubject(Setting::where(
    ...
    public function emailLoanSchedule($loan)
    {
        ...
        if (!empty($borrower->email)) {
            ...
            Mail::raw($body, function ($message) use ($borrower, $loan, $file_name): void {
                ...
                $headers = $message->getHeaders();
                $message->attach(
                    public_path() . '/uploads/temporary/loan_schedule' . $loan->id . '.pdf',
                    ['as' => $file_name]
                );
                $message->setContentType('text/html');
                $message->setSubject(Setting::where(
