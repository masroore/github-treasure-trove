...
class CronController extends Controller
{
    ...
    public function index()
    {
        if (Setting::where('setting_key', 'enable_cron')->first()->setting_value == 0) {
            ...
            Mail::raw(
                'Someone attempted to run con job but it is disabled, please enable it in settings',
            ...
            foreach ($schedules as $schedule) {
                ...
                if (!empty($schedule->borrower->email)) {
                    ...
                    $body = str_replace(
                        '{paymentAmount}',
                        round(
                                ($schedule->principal + $schedule->interest + $schedule->fees + $schedule->penalty),
                                2
                            ),
                        $body
                    );
                    ...
                    Mail::raw($body, function ($message) use ($borrower): void {
                        $message->from(
                            Setting::where('setting_key', 'company_email')->first()->setting_value,
                            Setting::where('setting_key', 'company_name')->first()->setting_value
                        );
        ...
        if (Setting::where('setting_key', 'auto_overdue_repayment_email_reminder')->first()->setting_value == 1) {
            ...
            foreach ($schedules as $schedule) {
                ...
                if (!empty($schedule->borrower->email)) {
                    $borrower = $schedule->borrower;
                    $loan = $schedule->loan;
                    $payments = LoanRepayment::where('loan_id', $loan->id)->where(
                        'due_date',
                        $schedule->due_date
                    )->sum('amount');
                    if ($payments == 0) {
        ...
        foreach ($loans as $loan) {
            ...
            if (!empty($product)) {
                //check for charges
                foreach (LoanProductCharge::where('loan_product_id', $product->id)->where(
                    'penalty',
                    1
                )->get() as $tkey) {
                    if (!empty($tkey->charge)) {
                        ...
                        if ($tkey->charge->charge_type == 'overdue_installment_fee') {
                            $schedule = LoanSchedule::where('loan_id', $loan->id)->where(
                                'due_date',
                                $date
                            )->first();
                            if (!empty($schedule)) {
                        ...
                        if ($tkey->charge->charge_type == 'overdue_maturity') {
                            ...
                            if ($loan->maturity_date < date('Y-m-d') && ($due_items['interest'] + $due_items['principal'] + $due_items['fees'] + $due_items['penalty'] - $paid_items['interest'] - $paid_items['principal'] - $paid_items['fees'] - $paid_items['penalty']) > 0) {
                                ...
                                $schedule = LoanSchedule::where('loan_id', $loan->id)->orderBy(
                                    'due_date',
                                    'desc'
                                )->first();
                                if (!empty($schedule)) {
        ...
        foreach ($expenses as $expense) {
            if (empty($expense->recur_end_date)) {
                if ($expense->recur_next_date == date('Y-m-d')) {
                    ...
                    $exp1->month = $date[1];
                    $exp1->save();
                    $custom_fields = CustomFieldMeta::where('parent_id', $expense->id)->where(
                        'category',
                        'expenses'
                    )->get();
                    ...
                    $exp2->recur_next_date = date_format(
                        date_add(
                        date_create(date('Y-m-d')),
        ...
        foreach ($payrolls as $payroll) {
            if (empty($payroll->recur_end_date)) {
                if ($payroll->recur_next_date == date('Y-m-d')) {
                    ...
                    $pay2->recur_next_date = date_format(
                        date_add(
                        date_create(date('Y-m-d')),
        ...
        foreach (SavingProduct::all() as $key) {
            //check post date
            foreach (Saving::where('savings_product_id', $key->id)->where(
                'status',
                'active'
            )->get() as $saving) {
                ...
                foreach (SavingsProductCharge::where('savings_product_id', $key->id)->get() as $tkey) {
                    if (!empty($tkey->charge)) {
                        ...
                        if ($tkey->charge->charge_type == 'specified_due_date') {
                            ...
                            foreach (SavingsCharge::where('savings_id', $saving->id)->where(
                                'charge_id',
                                $tkey->charge_id
                            )->where('date', date('Y-m-d'))->get() as $charge) {
                ...
                if (GeneralHelper::savings_account_balance($saving->id) >= $key->minimum_balance && $key->interest_rate > 0) {
                    ...
                    $last_interest = SavingTransaction::where('savings_id', $saving->id)->where(
                        'type',
                        'interest'
                    )->where('system_interest', 1)->orderBy('date', 'desc')->first();
                    ...
                    if (empty($last_interest)) {
                        ...
                        $diff = GeneralHelper::diff_in_months(
                        ...
                        if (str_replace(
                            ...
                            '',
                            date('d')
                        ) == $interest_adding && $diff >= $key->interest_posting && $diff <= $key->interest_posting + 1
                            ) {
                            $savings_transaction = new SavingTransaction();
                            $savings_transaction->borrower_id = $saving->borrower_id;
                            $savings_transaction->branch_id = $saving->branch_id;
        ...
        Setting::where(
            'setting_key',
            'cron_last_run'
        )->update(['setting_value' => date('Y-m-d H:i:s')]);
