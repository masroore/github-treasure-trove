...
class CommunicationController extends Controller
{
    ...
    public function storeEmail(Request $request)
    {
        ...
        if ($request->send_to == 0) {
            foreach (Borrower::all() as $borrower) {
                ...
                $body = str_replace(
                    '{borrowerTotalLoansPaid}',
                    GeneralHelper::borrower_loans_total_paid($borrower->id),
                    $body
                );
                $email = $borrower->email;
                if (!empty($email)) {
                    Mail::raw($body, function ($message) use ($request , $email): void {
                        $message->from(
    ...
    public function storeSms(Request $request)
    {
        ...
        return redirect('setting/data')->with(['error' => 'SMS is disabled, please enable it.']);
    }
