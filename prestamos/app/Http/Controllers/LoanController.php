...
class LoanController extends Controller
{
    ...
    public function indexRepayment()
    {
        ...
        $data = LoanTransaction::where(
            'transaction_type',
            'repayment'
        )->where('reversed', 0)->where('branch_id', session('branch_id'))->get();
...
//    print repayment
    public function pdfRepayment($loan_transaction): void
    {
        PDF::AddPage();
    ...
    public function emailBorrowerStatement($borrower)
    {
        if (!empty($borrower->email)) {
            ...
            Mail::raw($body, function ($message) use ($borrower, $file_name, $loans): void {
                $message->from(
    ...
    public function emailLoanStatement($loan)
    {
        ...
        if (!empty($borrower->email)) {
            ...
            Mail::raw($body, function ($message) use ($borrower, $loan, $file_name, $payments): void {
                $message->from(
                    Setting::where('setting_key', 'company_email')->first()->setting_value,
                    Setting::where('setting_key', 'company_name')->first()->setting_value
                );
    ...
    public function emailLoanSchedule($loan)
    {
        ...
        if (!empty($borrower->email)) {
            ...
            Mail::raw($body, function ($message) use ($borrower, $loan, $file_name, $schedules): void {
                $message->from(
                    Setting::where('setting_key', 'company_email')->first()->setting_value,
                    Setting::where('setting_key', 'company_name')->first()->setting_value
                );
