...
class UpdateInterestWaived
{
     ...
     * Handle the event.
     */
    public function handle(InterestWaived $event): void
    {
        ...
        $due_date = '';
        $count = 0;
        foreach (LoanTransaction::where('loan_id', $loan_transaction->loan_id)->where(
            'transaction_type',
            'waiver'
        )->where('reversed', 0)->orderBy('date', 'asc')->get() as $key) {
                        ...
                        foreach ($repayment_order as $order) {
                            if ($order == 'interest') {
                                if ($payments > $skey->interest) {
                                    ...
                                    foreach (LoanSchedule::where('loan_id', $loan_transaction->loan_id)->where(
                                        'due_date',
                                        '>',
                                        $skey->due_date
                                    )->orderBy('due_date', 'asc')->get() as $tkey) {
                                        if ($available > 0) {
