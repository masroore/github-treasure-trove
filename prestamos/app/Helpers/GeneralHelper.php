...
class GeneralHelper
{
    ...
    {
        $loan = Loan::find($id);
        $period = self::loan_period($id);
        $interest_rate = self::determine_interest_rate($id);
        //calculate here
        $amount = ($interest_rate * $balance * (1 + $interest_rate) ** $period) / ((1 + $interest_rate) **
                    $period - 1);
    ...
    {
        $schedule = LoanSchedule::where('due_date', ' >=', $date)->where('loan_id', $id)->orderBy(
            'due_date',
            'asc'
        )->first();
        if (!empty($schedule)) {
        ...
        $schedule = LoanSchedule::where('due_date', '>', $date)->where(
            'loan_id',
            $id
        )->orderBy(
                        'due_date',
    ...
    {
        if (empty($date)) {
            return self::loan_total_due_amount($id) - self::loan_total_paid($id);
        }

        return self::loan_total_due_amount($id, $date) - self::loan_total_paid(
            $id,
            $date
        );
    }
    ...
    public static function loan_total_due_amount($id, $date = '')
    {
        ...
        return self::loan_total_penalty($id, $date) + self::loan_total_fees(
            $id,
            $date
        ) + self::loan_total_interest($id, $date) + self::loan_total_principal(
                        $id,
                        $date
                    ) - self::loan_total_interest_waived($id, $date);
    }
    ...
    public static function addMonths($date, $months): void
    {
    ...
    public static function loan_terms_paid_item($id, $item = 'principal')
    {
        ...
        $penalty = 0;
        $fees = 0;
        $payments = self::loan_total_paid($id);
        $total_principal = self::loan_total_principal($id);
        $total_interest = self::loan_total_interest($id);
        $total_fees = self::loan_total_fees($id);
        $total_penalty = self::loan_total_penalty($id);
        if (!empty($loan->loan_product)) {
            $repayment_order = unserialize($loan->loan_product->repayment_order);
            if ($payments > 0) {
                foreach ($repayment_order as $order) {
    ...
    public static function loans_borrower_count($id, $start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            $due = 0;
            $due = $due + Loan::where('borrower_id', $id)->where(
                'branch_id',
                session('branch_id')
            )->whereIn(
                        'status',
    ...
    public static function asset_valuation($id, $start_date = '')
    {
        ...
        $value = 0;
        if (!empty(AssetValuation::where('asset_id', $id)->where('date', '<=', $start_date)->orderBy(
            'date',
            'desc'
        )->first())
            ) {
    ...
    public static function asset_type_valuation($id, $start_date = '')
    {
        ...
        $value = 0;
        foreach (Asset::where('asset_type_id', $id)->get() as $key) {
            if (!empty(AssetValuation::where('asset_id', $key->id)->where(
                'date',
                '<=',
                $start_date
            )->orderBy(
    ...
    public static function send_sms($to, $msg): void
    {
        if (Setting::where('setting_key', 'sms_enabled')->first()->setting_value == 1) {
            if (!empty(SmsGateway::find(Setting::where(
                'setting_key',
                'active_sms'
            )->first()->setting_value))
            ) {
    ...
    public static function printTree($tree, $r = 0, $p = null): void
    {
        foreach ($tree as $i => $t) {
    ...
    public static function loan_allocate_payment($loan_transaction)
    {
        ...
        if (!empty($loan->loan_product)) {
            ...
            $payments = LoanTransaction::where('loan_id', $loan_transaction->loan_id)->where(
                'transaction_type',
                'repayment'
            )->where('reversed', 0)->where('date', '<=', $loan_transaction->date)->sum('credit') - $amount;
    ...
    public static function loan_schedule_dtermine_paid_by($id): void
    {
    ...
    public static function loan_paid_items($id, $start_date = '', $end_date = '')
    {
        ...
        if (!empty($loan->loan_product)) {
            if (empty($start_date)) {
                $payments = LoanTransaction::where('loan_id', $id)->where(
                    'transaction_type',
                    'repayment'
                )->where('reversed', 0)->sum('credit');
                $interest_waived = LoanTransaction::where('loan_id', $id)->where(
                    'transaction_type',
                    'waiver'
                )->where('reversed', 0)->sum('credit');
            } else {
