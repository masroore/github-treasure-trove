...
class CategoryRepository
{
    ...
    public function detachTasks(Category $category): void
    {
    ...
    public function addToTeam(Team $team, Category $category): void
    {
        $team->categories()->attach($category->id);
    }

    public function addToUser(User $user, Category $category): void
    {
    ...
    public function getChildTasks(Category $category, Team $team, User $user, array $currentTaskIds, $search = null, $withTrashed = false, $onlyTrashed = false)
    {
        ...
        $category->load(['descendants' => function ($q) use ($team, $user, $search): void {
            $q->with(['tasks' => function ($q2) use ($team, $user, $search): void {
                $q2->filter($search);
                $q2->forList($team, $user);
                $q2->withTrashed();
            }]);
    ...
    {
        $category->load(['descendants' => function ($q) use ($user, $search, $withTrashed, $onlyTrashed, $currentTaskIds): void {
            if ($onlyTrashed) {
                $q->with(['tasks' => function ($q2) use ($user, $search, $withTrashed , $currentTaskIds): void {
                    $q2->onlyTrashed();
                    $q2->whereNotIn('id', $currentTaskIds);
                    $q2->filter($search);
    ...
    {
        $category->load(['text', 'subscribers', 'histories', 'comments' => function ($q): void {
    ...
    {
        return $category->load(['text', 'histories' => function ($q): void {
            $q->orderBy('created_at', 'desc');
        }]);
    }

    public function addCategoryHistory(Category $category, User $user): void
    {
        $data = [
    ...
    public function forceDeleteWithRelations(Category $category): void
    {
