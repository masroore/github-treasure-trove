...
class UserRepository
{
    ...
    {
        return $this->mainModel->whereHas('hashes', function ($q) use ($hash, $type): void {
            $q->where('hash', $hash);
            $q->where('type', $type);
        })->first();
    }
    ...
    {
        return User::whereHas('key', function ($q) use ($key): void {
            $q->where('key', $key);
        })->first();
    }
    ...
    public function deleteUserHash(User $user, string $type): void
    {
        $user->hashes()->where('type', $type)->get()->each(function ($value): void {
            $value->delete();
        });
    }
    ...
    {
        return User::whereDoesntHave('teams', function ($q) use ($team): void {
    ...
    {
        return User::where(function ($q) use ($user, $team): void {
            $q->where('id', '<>', $user->id);
            $q->whereDoesntHave('teams', function ($q2) use ($team): void {
    ...
    public function addFile(User $user, array $data): void
    {
    ...
    {
        $user->load(['teams' => function ($q) use ($user): void {
    ...
    {
        return $user->readComments()->whereHas('tasks', function ($q) use ($task): void {
    ...
    public function fullDeleteAccount(User $user): void
    {
