...
class Category extends Model
{
    ...
    {
        return $query->whereHas('users', function ($q) use ($user): void {
    ...
    public function scopeForTree($query, User $user)
    {
        return $query->withCounts($user)->with([
            'text',
            'descendants' => function ($q) use ($user): void {
                $q->withCounts($user);
            },
        ]);
    }

    public function scopeWithCounts($query, User $user): void
    {
        $query->orderBy('name');
        $query->withCount([
            'categoryNotifications',
            'userNotifications' => function ($q2) use ($user): void {
    ...
    {
        return $query->with(['tasks' => function ($q) use ($user): void {
