...
class ActivityService extends CoreService
{
    ...
    public function taskEdited(User $user, Team $team, Task $task, array $sync, array $original, bool $changesText): void
    {
        $task = $team->tasks()->withTrashed()->withAuthorUsername($team)->with(['users' => function ($q) use ($team): void {
            $q->withUsername($team);
