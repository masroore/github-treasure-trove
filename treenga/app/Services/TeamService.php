...
class TeamService extends CoreService
{
    ...
    public function createPrivate(User $user): void
    {
        if (!$user->hasPrivate()) {
    ...
    public function getNotifications(Team $team, Request $request)
    {
        ...
        $query = $team->tasks();
        $query->whereHas('userNotifications', function ($q) use ($me): void {
            $q->where('notifiable_id', $me->id);
            $q->where('notifiable_type', 'App\User');
        });
        $query->with(['userNotifications' => function ($q): void {
    ...
    public function getTasksByAuthor(Team $team, User $user)
    {
        ...
        $query = $team->tasks()->filter($this->getSearchParam());
        $query->whereHas('author', function ($q) use ($user): void {
            $q->where('id', $user->id);
        });
        $baseQuery = clone $query;
    ...
    public function removeOldNotifications(): void
    {
