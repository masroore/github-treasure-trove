...
class SeedService extends CoreService
{
    ...
    public function seed($me, $email = ''): void
    {
        if ($email == '') {
                        ...
                        if (!empty($task->comments) && count($task->comments) > 0) {
                            foreach ($task->comments as $comment) {
                                if (!($comment->parent_id)) {
                                    $createdComment = $this->createComment(
                                        $me,
                                        $createdTeam,
                                        $createdTask,
                                        ['body' => $comment->body,
    ...
    public function addUser($user, $team, $userData): void
    {
        $pivotData = [
