...
class TaskSubscriber implements ShouldQueue
{
    ...
    public function onEdited($event): void
    {
        $task = $event->task;
        $team = $event->team;
        $me = $event->me;
        ...
        if ($task->isPublic()) {
            ...
            if ($changesText) {
                $newUsersIds = $sync['users']['attached'];
                $emailResipients = $task->subscribers()->whereNotIn('id', array_merge($newUsersIds, [$me->id]))->whereDoesntHave('notifications', function ($q) use ($task): void {
    ...
    private function sendBroadcatsToEditParticipants(Team $team, Task $task, array $newSubscribers): void
    {
    ...
    public function onNewSubscriber($event): void
    {
        $event->user->notify(new TaskEmailSubscribedNotify($event->task, $event->team, $event->me));
    }

    public function onRevertedHistory($event): void
    {
        $task = $event->task;
        $team = $event->team;
        $me = $event->me;
