...
class Task extends Model
{
    ...
    public function scopeFilter($query, $search = null)
    {
        if ($search) {
            ...
            return $query->where(function ($q1) use ($search, $textsIds): void {
                $q1->whereHas('text', function ($q2) use ($textsIds): void {
    ...
    {
        return $query->with(['author.teams' => function ($q2) use ($team): void {
    ...
    public function scopeForList($query, Team $team, User $user)
    {
        ...
        $query->withCount(['tempItem' => function ($q) use ($user): void {
            $q->where('user_id', $user->id);
        }]);
        $query->withCount(['userNotifications' => function ($q) use ($user): void {
            $q->where('user_id', $user->id);
    ...
    {
        return $query->where('type', self::TYPE_PUBLIC)->orWhere(function ($q) use ($user): void {
