...
class RenderFromDatabaseData
{
    ...
    private function addTitle($data): void
    {
        $this->mb->addTitle($data['id'], $data['name'], '', '', $data['sequence']);
    }

    private function addLink($data): void
    {
    ...
    private function dropdownLoop($id): void
    {
        for ($i = 0; $i < count($this->data); ++$i) {
            if ($this->data[$i]['parent_id'] == $id) {
                if ($this->data[$i]['slug'] === 'dropdown') {
                    ...
                    $this->mb->addLink($this->data[$i]['id'], $this->data[$i]['name'], $this->data[$i]['href'], '', '', $this->data[$i]['sequence']);
                } else {
    ...
    private function addDropdown($data): void
    {
