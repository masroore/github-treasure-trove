        ...
        <a href="{{ route('upsos.show', [
            ...
            <div class="{{ isset( $upso) && $upso->id == $post->id ? 'bg-gray-400  border border-red-600': '' }} flex items-center justify-between shadow-md text-xs my-2 ">
                <div class="flex items-center justify-start">
                    {{-- <img src="{{ $image_server  . $post->thumb_path }}" class="w-24 h-20 sm:w-32 sm:h-24 mx-1"> --}}
                    <div class="">
                        <div class="flex my-1">
                            <div class="p-2 mx-1 w-24 bg-gray-100 text-black rounded leading-none text-center border border-gray-500 ">
                                {{  $post->upso_type->title }}
                            </div>
                            <div class="p-2 mx-1 w-32 bg-gray-100 text-black rounded leading-none text-center border border-gray-500 ">
                                {{  $post->region->title }}
                            </div>
                        </div>
                        <div class="flex w-48 my-1">
                            <div class="p-2 mx-1  bg-my-black flex-grow upso-title text-white rounded leading-none text-center">
                                {{  $post->site_name }}
                            </div>
                        </div>
