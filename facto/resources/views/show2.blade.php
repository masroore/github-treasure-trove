...
<div class="p-1 mb-5 ">
    ...
    <div class="relative aspect-16x9 mt-8 " >
            ...
            <iframe width="560" height="315" src="//sendvid.com/embed/v4punwi1" frameborder="0" allowfullscreen></iframe>

        {{-- <iframe src="{{ env('APP_DEV') == 'TEST'? '':  $post->outlink1 }}" class="absolute w-full h-full border border-indigo-400"></iframe> --}}
        {{-- <iframe src="{{   $post->outlink1 }}" class="absolute w-full h-full border border-indigo-400 w-48" ></iframe> --}}
    </div>
...
<div class="w-full mx-3 my-2 border border-gray-400 ">
            ...
            <div class=" rounded h-auto  ">
                ...
                <div class=" p-1 rounded rounded-lg  spect-4x3">
                        <a href="/posts/{{ $item->id}}?page={{ $page}}&tag_id={{ isset( $tag) ? $tag->id : -1 }}">
                            <img class=" hidden sm:block w-full object-cover rounded  "  style="max-height:150px;"
                                src="{{ env('APP_DEV') === 'TEST' ? '/storage/' . $item->thumb_path : '/'. $item->thumb_path }}"
                                 />

                            <img class="sm:hidden w-full object-cover rounded  "  style="max-height:100px;"
                                src="{{ env('APP_DEV') === 'TEST' ? '/storage/' . $item->thumb_path : '/'. $item->thumb_path }}"
                                />       
