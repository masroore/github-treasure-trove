    ...
    <div class="flex w-full items-center justify-between text-xs font-light text-gray-700 px-2">
        ...
        <div class="flex items-center ">
            ...
            <div class="mx-1 text-red-600">
                {{ \Carbon\Carbon::parse( $post->created_at)->locale('ko_KR')->diffForhumans() }}</div>
        </div>
    </div>
...
<div class="p-1 mb-5 ">
    ...
    @if( Str::of( $post->outlink1 )->contains('iframe'))
        <div class="w-full h-full flex flex-col items-center justify-center my-6">
            {!! $post->outlink1 !!}
        </div>
    @else 
        <div class="relative aspect-16x9 mt-8 ">
            <iframe src="{{   $post->embed }}" class="absolute w-full h-full border border-indigo-400 w-48" frameborder="0"
...
<div class="w-full mx-0 my-2 border border-gray-400 ">
    ...
    <div class="flex flex-wrap ">
        ...
        <div class="flex-initial mr-2">
            <div
                class="text-gray-700 text-center p-2 text-xs font-light {{ isset( $tag) &&  $tag->id == $item->id ? 'text-red-500': '' }} ">
            ...
            <div class=" rounded h-auto object-none object-top ">
                ...
                <div class=" p-1 rounded rounded-lg  spect-4x3">
                    ...
                    <a href="{{ route('posts.show', array_merge( ['post'=>$item ], request()->all() )  ) }}">
                        <img class=" hidden sm:block w-full object-cover rounded  cursor-pointer "
                            style="min-height:100px; max-height:150px;"
                            src="{{ env('APP_DEV') === 'TEST' ? '/storage/' . $item->thumb_path : '/'. $item->thumb_path }}" />

                        <img class="sm:hidden w-full object-cover rounded  cursor-pointer " 
