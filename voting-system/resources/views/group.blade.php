    ...
    @else
        ...
        <h4 class="text-center mt-4">{{ $season->name }} - <span>{{ $stage->name }}</span><br>
            ...
            @if ($nowDate->gte($stage->start_date) && $nowDate->lte($stage->end_date))
                @php
                    if (Auth::user()) {
                        $check = App\Models\Votes::where('group_id', $group->id)
                    ...
                    $nominees = App\Models\Nominations::with('artist')->where('group_id', $group->id)->where('season_id', $season->id)->where('stage_id', $stage->id)->where('status', 0)->get();

                @endphp
                <div class="characteristics">
                    <div class="container">
                        <h4 class="text-center mb-4">{{ $group->name }} (<span>{{ count($nominees) }}
                                Nominees)</span>
                        </h4>
                        ...
                        <div class="row">
                            ...
                            @foreach ($nominees as $value)
                                @php
                                    if (Auth::user()) {
                                        $checkArtist = App\Models\Votes::where('group_id', $group->id)
                                            ->where('stage_id', $stage->id)
                                ...
                                @endphp
                                <!-- Char. Item -->
                                <div class="col-lg-3 col-md-6 char_col mb-2 artist_{{ $group->id }}_{{ $value->artist->id }}"
                                    style="cursor: pointer">
                                    <div class="char_item d-flex flex-row align-items-center justify-content-start artist_{{ $group->id }}_{{ $value->artist->id }} @if (Auth::user() && $checkArtist) voted @endif
                                        @if (Auth::user() && $check) @else vote @endif"
                                        data-group="{{ $group->id }}" data-artist="{{ $value->artist->id }}"
                                        data-name="{{ $value->artist->name }}" data-group="{{ $group->id }}">
                                        <div class="char_icon"><img src="{{ asset('assets/images/char_4.png') }}" alt="">
                                        </div>
                                        <div class="char_content">
                                            <div class="char_title"><a href="#">{{ $value->artist->name }}</a>
                                            </div>
