...
class order
{
    ...
    {
        return Helpers::order_data_formatting(self::with(['details', 'delivery_man.rating'])->where(['id' => $order_id])->first(), false);
    }
    ...
    public static function place_order($customer_id, $email, $customer_info, $cart, $payment_method, $discount, $coupon_code = null)
    {
        try {
            $or = [
                ...
                'discount_type' => 0 == $discount ? null : 'coupon_discount',
            ...
            foreach ($cart as $c) {
                ...
                $or_d = [
                    'order_id' => $o_id,
                    'product_id' => $c['id'],
                    'seller_id' => 'seller' == $product->added_by ? $product->user_id : '0',
