...
class helpers
{
            ...
            foreach ($data as $item) {
                ...
                if (\count($item['translations'])) {
                    foreach ($item['translations'] as $translation) {
                        if ('name' == $translation->key) {
                            $item['name'] = $translation->value;
                        }
                        if ('description' == $translation->key) {
    ...
    {
        return Currency::where(['currency_code' => self::currency_code()])->first()->currency_symbol;
    }
    ...
    {
        if ('percent' == $product['discount_type']) {
    ...
    public static function get_delivery_charge($distance)
    {
        ...
        if (1 != $config['status']) {
            return BusinessSetting::where(['key' => 'delivery_charge'])->first()->value;
        }
        $delivery_charge = 0;
