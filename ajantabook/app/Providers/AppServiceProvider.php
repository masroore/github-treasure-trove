    ...
    public function boot(): void
    {
        ...
        try {
            ...
            if (Schema::hasTable('genrals') && Schema::hasTable('seos') && Schema::hasTable('theme_settings')) {
                ...
                View::composer('front.checkout', function ($view): void {
                    $view->with([
                        'cart_table' => Auth::user()->cart,
                    ]);
                });

                View::composer('front.layout.master', function ($view): void {
                ...
                View::composer('front.layout.header', function ($view) use ($data): void {
                    $total = 0;
                    ...
                    if (Auth::check()) {
                        ...
                        foreach ($carts as $key => $val) {
                            if (null != $val->semi_total && 0 != $val->semi_total && '' != $val->semi_total) {
                ...
                view()->composer('*', function ($view) use ($data): void {
                    try {
                        $view->with([
