...
class FlashdealController extends Controller
{
    ...
    public function dealproducts($deal)
    {
        ...
        foreach ($dealitems as $key => $vp) {
            ...
            if (isset($vp->variant, $vp->variant->products) && '1' == $vp->variant->products->status) {
                ...
                $sellprice = 0 != $price->offerprice ? $price->offerprice : $price->mainprice;
                ...
                if ('upto' == $discount_type) {
                    $random_no = mt_rand(0, $discount);
                ...
                $images = $vp->variant->variantimages()->select('image1', 'image2', 'image3', 'image4', 'image5', 'image6')->get()->map(function ($image) {
                    if (null != $image->image1) {
                        $item[]['image'] = $image->image1;
                    }

                    if (null != $image->image2) {
                        $item[]['image'] = $image->image2;
                    }

                    if (null != $image->image3) {
                        $item[]['image'] = $image->image3;
                    }

                    if (null != $image->image4) {
                        $item[]['image'] = $image->image4;
                    }

                    if (null != $image->image5) {
                        $item[]['image'] = $image->image5;
                    }

                    if (null != $image->image6) {
                ...
                $products[] = [
                    ...
                    'discount' => 'upto' == $vp->discount_type ? __('Upto :discount% discount', ['discount' => $vp->discount]) : __(':discount% flat discount', ['discount' => $vp->discount]),
                    'images' => $images[0],
            ...
            if (isset($vp->simple_product) && '1' == $vp->simple_product->status) {
                $sellprice = 0 != $vp->simple_product->offer_price ? $vp->simple_product->offer_price : $vp->simple_product->price;
