...
class WishlistController extends Controller
{
    ...
    public function listCollection()
    {
        ...
        $collection->each(function ($coll): void {
            $result[] = [
                'id' => $coll->id,
    ...
    public function topCollectionItemImages($coll)
    {
        ...
        if ('0' != $coll) {
            foreach ($coll->items->take(4) as $item) {
                if (isset($item->variant, $item->variant->variantimages)) {
                    $images[] = $item->variant->variantimages['main_image'];
                }
            }
        } else {
            auth()->user()->wishlist()->take(6)->each(function ($item): void {
                if (isset($item->variant, $item->variant->variantimages)) {
                    $images[] = $item->variant->variantimages['main_image'];
                }
            });
    ...
    public function collectionItem($collection, $currency)
    {
        ...
        foreach ($collection->items as $item) {
            ...
            $wishlistItem[] = [
                ...
                'stock' => 0 != $item->variant->stock ? 'In Stock' : 'Out of Stock',
                'rating' => (float) $rating,
                'symbol' => $rate->symbol,
            ];
        }
