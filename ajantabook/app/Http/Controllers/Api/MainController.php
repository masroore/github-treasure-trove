...
class MainController extends Controller
{
    ...
    public function homepage(Request $request)
    {
        ...
        $blogs = [
            ...
            'items' => $this->gettingBlogs($content = []),
        ];
        ...
        $homepage = [
            ...
            'sliders' => $sliders,
            'TwoEqualAdvertise' => null != $this->advertise('abovenewproduct', 'Two equal image layout') ? $this->advertise('abovenewproduct', 'Two equal image layout') : null,
            'hotdeals' => $this->hotdeals($request, $content),
            'featuredProducts' => $this->featuredProducts($content),
            'ThreeEqualAdvertise' => null != $this->advertise('afterfeaturedproduct', 'Three Image Layout') ? $this->advertise('afterfeaturedproduct', 'Three Image Layout') : null,
            'topCatgories' => $topcategories,
            'SingleAdvertise' => null != $this->advertise('abovelatestblog', 'Single Image Layout') ? $this->advertise('abovelatestblog', 'Single Image Layout') : null,
            'brands' => $this->brandSlider($request),
            'TwoNonEqualAdvertise' => null != $this->advertise('abovenewproduct', 'Two non equal image layout') ? $this->advertise('abovenewproduct', 'Two non equal image layout') : null,
            'blogs' => $blogs,
        ...
        $homepage_sections = []; // the second array
    ...
    public function advertise($position, $type)
    {
        ...
        foreach ($adv as $ad) {
            ...
            if ('Two non equal image layout' == $type) {
                $linkby = '';
            ...
            if ('Two equal image layout' == $type) {
                $linkby = '';
    ...
    public function slider($content)
    {
        ...
        foreach ($sliders as $key => $slider) {
            ...
            if ('cat' == $slider->link_by) {
                $type = 'category';
            } elseif ('sub' == $slider->link_by) {
                $type = 'subcategory';
            } elseif ('url' == $slider->link_by) {
                $type = 'subcategory';
            } else {
                $type = 'None';
            }
            ...
            $content[] = [
                ...
                'moredescription' => null != $slider->moredesc ? $slider->moredesc : 'Not found',
    ...
    public function tabbedProducts()
    {
        ...
        if (isset($frontcat) && '' != $frontcat->name && '1' == $frontcat->status) {
            $tabbed[] = [
                'tabid' => '0',
                'tabname' => [
                    'en' => __('ALL'),
                ],
                'products' => null != $this->recentProducts() ? $this->recentProducts() : 'No Products found',
            ];
    ...
    public function recentProducts()
    {
        ...
        $topcatproducts = Product::with('category')->whereHas('category', function ($q): void {
            $q->where('status', '=', '1');
        ...
        $topcatproducts = $topcatproducts->map(function ($q) use ($review, $wishlist) {
            ...
            if (0 != $this->getprice($q, $orivar)->getData()->offerprice) {
            ...
            $item['rating'] = (float) get_product_rating($q->id);
        ...
        $top_simple_products = SimpleProduct::with('category')->whereHas('category', function ($q): void {
            $q->where('status', '=', '1');
        })->with('subcategory')->wherehas('subcategory', function ($q): void {
            $q->where('status', '1');
        })->with('store')->whereHas('store', function ($query) {
    ...
    public function categoryproducts($catid)
    {
        ...
        if (isset($category)) {
            ...
            $categoryproducts_vp = Product::with('category')->whereHas('category', function ($q) use ($category): void {
            ...
            $categoryproducts_sp = SimpleProduct::with('category')->whereHas('category', function ($q) use ($category): void {
    ...
    public function featuredProducts($content)
    {
        ...
        $featured_variant_products = Product::with('category')->whereHas('category', function ($q): void {
            $q->where('status', '=', '1');
        })->with('subcategory')->wherehas('subcategory', function ($q): void {
            $q->where('status', '1');
        })->with('vender')->whereHas('vender', function ($query) use ($sellerSystem): void {
        ...
        $featured_simple_products = SimpleProduct::with('category')->whereHas('category', function ($q): void {
            $q->where('status', '=', '1');
        })->with('subcategory')->wherehas('subcategory', function ($q): void {
            $q->where('status', '1');
        })->with('store')->whereHas('store', function ($query) {
    ...
    public function getsubcategoryproduct(Request $request, $id)
    {
        ...
        $categoryproducts_vp = Product::with('category')->whereHas('category', function ($q) use ($subcategory): void {
            $q->where('status', '=', '1')->where('id', $subcategory->category->id);
        })->with('subcategory')->wherehas('subcategory', function ($q) use ($subcategory): void {
        ...
        $categoryproducts_sp = SimpleProduct::with('category')->whereHas('category', function ($q) use ($subcategory): void {
    ...
    public function getchildcategoryproduct(Request $request, $id)
    {
        ...
        $categoryproducts_vp = Product::with('category')->whereHas('category', function ($q) use ($childcat): void {
            $q->where('status', '=', '1')->where('id', $childcat->subcategory->category->id);
        })->with('subcategory')->wherehas('subcategory', function ($q) use ($childcat): void {
            $q->where('status', '1')->where('id', $childcat->subcategory->id);
        })->with('childcat')->wherehas('childcat', function ($q) use ($childcat): void {
        ...
        $categoryproducts_sp = SimpleProduct::with('category')->whereHas('category', function ($q) use ($childcat): void {
    ...
    public function specialoffer($content)
    {
        ...
        $vp_specialoffers = $vp_specialoffers->map(function ($sp) {
            ...
            if (0 != $this->getprice($sp, $orivar)->getData()->offerprice) {
            ...
            $content['variantid'] = $orivar->id;
            $content['type'] = 'v';
            $content['mainprice'] = (float) sprintf('%.2f', $price->mainprice * $this->rate->exchange_rate);
            $content['offerprice'] = (float) sprintf('%.2f', $price->offerprice * $this->rate->exchange_rate);
            $content['pricein'] = $this->rate->code;
            $content['symbol'] = $this->rate->symbol;
            $content['rating'] = (float) ProductRating::getReview($sp);
            $content['thumbnail'] = $orivar->variantimages->main_image;
    ...
    public function getVariant($orivar)
    {
        ...
        foreach ($orivar->main_attr_value as $key => $orivars) {
            ...
            if ($i < $varcount) {
                if (isset($getvarvalue) && 0 != strcasecmp($getvarvalue->unit_value, $getvarvalue->values) && null != $getvarvalue->unit_value) {
