...
class CartController extends Controller
{
    ...
    public function getShipping($qty, $variant)
    {
        ...
        if (0 == $variant->products->free_shipping) {
    ...
    public function yourCart(Request $request)
    {
        ...
        if (\count($this->cartproducts($request->currency))) {
            if (0 != $cart_free_shipping || '' != $cart_free_shipping) {
            ...
            $cart = [
                ...
                'currency' => $rate->code,
                'symbol' => $rate->symbol,
                'appliedCoupan' => null != $this->appliedCoupan($rate) ? $this->appliedCoupan($rate)->getData() : null,
    ...
    public function cartproducts($currency)
    {
        ...
        foreach (Auth::user()->cart as $cart) {
            if (isset($cart->variant, $cart->variant->products) && 1 == $cart->variant->products->status) {
                ...
                if (0 != $productData->getprice($cart->product, $cart->variant)->getData()->offerprice) {
            ...
            if (isset($cart->simple_product) && 1 == $cart->simple_product->status) {
                if (0 != $cart->simple_product->offer_price) {
    ...
    public function calculateShipping()
    {
        ...
        foreach (Auth::user()->cart as $cart) {
            if ($cart->variant && $cart->variant->products && 1 == $cart->variant->products->status) {
                $shipping += ShippingPrice::calculateShipping($cart);
            }

            if ($cart->simple_product && 1 == $cart->simple_product->status) {
                $shipping += shippingprice($cart);
            }
        }
    ...
    public function variantDetail($variant)
    {
        ...
        foreach ($variant->main_attr_value as $key => $orivars) {
            ...
            $variants[] = [
                ...
                'type' => 'color' == $loopgetattrname['attr_name'] || 'Color' == $loopgetattrname['attr_name'] || 'colour' == $loopgetattrname['attr_name'] || 'Colour' == $loopgetattrname['attr_name'] ? 'c' : 's',
            ];
        }
    ...
    public function increaseQuantity(Request $request)
    {
        ...
        if (isset($cartrow->variant, $cartrow->variant->products) && 1 == $cartrow->variant->products->status) {
        ...
        if (isset($cartrow->simple_product) && 1 == $cartrow->simple_product->status) {
            ...
            if (isset($cart->variant, $cart->variant->products) && 1 == $cart->variant->products->status) {
                ...
                foreach ($coupans as $c) {
                    if (0 != $c->maxusage) {
                        ...
                        $content[] = [
                            'coupanid' => $c->id,
                            'code' => $c->code,
                            'discount' => 'fix' == $c->distype ? (float) sprintf('%.2f', $c->amount * $rate->exchange_rate) : (int) $c->amount,
                            ...
                            'validationtext' => null != $this->findOfferText($c, $rate)->getData()->validationText ? $this->findOfferText($c, $rate)->getData()->validationText : null,
                        ];
                    }
                }
                ...
                foreach ($productcoupans as $c1) {
                    ...
                    $content[] = [
                        'coupanid' => $c1->id,
                        'code' => $c1->code,
                        'discount' => 'fix' == $c1->distype ? (float) sprintf('%.2f', $c1->amount * $rate->exchange_rate) : (int) $c1->amount,
                        ...
                        'validationtext' => null != $this->findOfferText($c1, $rate)->getData()->validationText ? $this->findOfferText($c1, $rate)->getData()->validationText : null,
                    ];
                }
                ...
                foreach ($productcategorycoupans as $c2) {
                    ...
                    $content[] = [
                        'coupanid' => $c2->id,
                        'code' => $c2->code,
                        'discount' => 'fix' == $c2->distype ? (float) sprintf('%.2f', $c2->amount * $rate->exchange_rate) : (int) $c2->amount,
                        ...
                        'validationtext' => null != $this->findOfferText($c2, $rate)->getData()->validationText ? $this->findOfferText($c2, $rate)->getData()->validationText : null,
                    ];
                }
            }
            ...
            if (isset($cart->simple_product) && 1 == $cart->simple_product->status) {
                ...
                if (isset($simple_products_coupans)) {
                    foreach ($simple_products_coupans as $sc1) {
                        ...
                        $content[] = [
                            'coupanid' => $sc1->id,
                            'code' => $sc1->code,
                            'discount' => 'fix' == $sc1->distype ? (float) sprintf('%.2f', $sc1->amount * $rate->exchange_rate) : (int) $sc1->amount,
                            ...
                            'validationtext' => null != $this->findOfferText($sc1, $rate)->getData()->validationText ? $this->findOfferText($sc1, $rate)->getData()->validationText : null,
                        ];
                    }
                }
                ...
                foreach ($simpleproductcategorycoupans as $sc2) {
                    ...
                    $content[] = [
                        'coupanid' => $sc2->id,
                        'code' => $sc2->code,
                        'discount' => 'fix' == $sc2->distype ? (float) sprintf('%.2f', $sc2->amount * $rate->exchange_rate) : (int) $sc2->amount,
                        ...
                        'validationtext' => null != $this->findOfferText($sc2, $rate)->getData()->validationText ? $this->findOfferText($sc2, $rate)->getData()->validationText : null,
                    ];
                }
            }
        }
    ...
    public function appliedCoupan($rate)
    {
        ...
        if ($cpn) {
            ...
            return response()->json([
                'coupanid' => $cpn->id,
                'code' => $cpn->code,
                'discount' => 'fix' == $cpn->distype ? (float) sprintf('%.2f', $cpn->amount * $rate->exchange_rate) : (int) $cpn->amount,
    ...
    public function guestCart(Request $request)
    {
        ...
        if ($request->cart) {
            $cart = [
                ...
                'subtotal' => (float) sprintf('%.2f', $this->guestcartTotal()->getData()->subtotal * $rate->exchange_rate),
                'shipping' => (float) sprintf('%.2f', $this->guestcartTotal()->getData()->shipping * $rate->exchange_rate),
                'grand_total' => (float) sprintf('%.2f', $this->guestcartTotal()->getData()->grandTotal * $rate->exchange_rate),
                'currency' => $rate->code,
                'symbol' => $rate->symbol,
        ...
        foreach (request()->cart as $cart) {
            ...
            if (isset($cart['variantid'])) {
                ...
                if (isset($variant)) {
                    ...
                    if (0 != $productData->getprice($variant->products, $variant)->getData()->offerprice) {
    ...
    public function guestcartTotal()
    {
        ...
        foreach (request()->cart as $cart) {
            if (isset($cart['variantid'])) {
                ...
                if (isset($variant)) {
                    if (0 != $variant->products->offer_price) {
    ...
    public function guestCartCalculateShipping()
    {
        ...
        foreach (request()->cart as $cart) {
            if (isset($cart['variantid'])) {
                ...
                if (isset($variant, $variant->products)) {
                    if ('0' == $variant->products->free_shipping) {
