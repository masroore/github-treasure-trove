...
class ProductController extends Controller
{
    public function detailProduct(Request $request, $productid, $variantid)
    {
        ...
        foreach ($pro->subvariants as $key => $othervariant) {
            ...
            if (0 != $this->getprice($pro, $othervariant)->getData()->offerprice) {
            ...
            $combinations[] = [
                ...
                'mainprice' => (float) sprintf('%.2f', $this->getprice($pro, $othervariant)->getData()->mainprice * $this->rate->exchange_rate),
                'offerprice' => (float) sprintf('%.2f', $this->getprice($pro, $othervariant)->getData()->offerprice * $this->rate->exchange_rate),
        ...
        if (1 == $product->products->free_shipping) {
            $otherservices[] = [
                'type' => 'freeshipping',
        ...
        if (1 == $product->products->return_avbl) {
            $otherservices[] = [
                'type' => 'return',
        ...
        if (1 == $product->products->codcheck) {
            $otherservices[] = [
                'type' => 'cod',
    ...
    public function getRelatedProducts($product)
    {
        ...
        if (isset($product->subvariants)) {
            if ('1' == $product->relsetting->status) {
    ...
    public function wishlist(Request $request)
    {
        ...
        $wishlistItem = $data_vp->map(function ($item) use ($rate) {
            ...
            if ('0' != $this->getprice($item->variant->products, $item->variant)->getData()->offerprice) {
            ...
            $content['price'] = (float) sprintf('%.2f', $price->mainprice * $rate->exchange_rate);
            $content['offerprice'] = (float) sprintf('%.2f', $price->offerprice * $rate->exchange_rate);
            $content['stock'] = 0 != $item->variant->stock ? 'In Stock' : 'Out of Stock';
        ...
        $data_sp = $data_sp->map(function ($sp) use ($rate) {
            if (0 != $sp->simple_product->offer_price) {
            ...
            $item['variantid'] = 0;
            $item['type'] = 's';
            $item['mainprice'] = (float) sprintf('%.2f', $sp->simple_product->price * $rate->exchange_rate);
            $item['offerprice'] = (float) sprintf('%.2f', $sp->simple_product->offer_price * $rate->exchange_rate);
            $item['pricein'] = $rate->code;
            $item['symbol'] = $rate->symbol;
            $item['rating'] = (float) simple_product_rating($sp->simple_product->id);
            $item['thumbnail'] = $sp->simple_product->thumbnail;
            $item['thumb_path'] = url('images/simple_products/');
            $item['off_in_percent'] = round($offamount);
            $item['rating'] = (float) simple_product_rating($sp->simple_product->id);
            $item['stock'] = 0 != $sp->simple_product->stock ? 'In Stock' : 'Out of Stock';
    ...
    {
        return UserReview::where('pro_id', $pro->id)->where('review', '!=', null)->where('status', '1')->get();
    }
