...
class FlashSaleController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($deals)
                ...
                ->addColumn('background_image', function ($row) {
                    return '<img style="object-fit:scale-down;" width="100px" src="' . url('/images/flashdeals/' . $row->background_image) . '"/>';
                })
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->product as $key => $product) {
            $newdeal->saleitems()->create([
                'sale_id' => $newdeal->id,
                'product_id' => 'variant' == $request->type[$key] ? $request->product_id[$key] : 0,
                'simple_product_id' => 'simple' == $request->type[$key] ? $request->product_id[$key] : 0,
    ...
    public function searchproduct(Request $request)
    {
        ...
        if (!\count($query)) {
            $query[] = ['value' => 'No result found', 'product_type' => '0', 'id' => 0];
        }
