...
class VenderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (empty($cat)) {
            if ($file = $request->file('store_logo')) {
                ...
                $optimizeImage->resize(396, 396, function ($constraint): void {
                    $constraint->aspectRatio();
                });
    ...
    public function destroy($id)
    {
        ...
        if ($store) {
            ...
            DB::table('products')
                ->where('vender_id', $auth_id)->update([
                    'status' => '0',
                ]);
        }
    ...
    {
        if (Auth::check() && 'a' == Auth::user()->role_id) {
