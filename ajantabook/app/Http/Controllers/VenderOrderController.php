...
class VenderOrderController extends Controller
{
    ...
    {
        if (Auth::check()) {
            if ('v' == Auth::user()->role_id || 'a' == Auth::user()->role_id) {
                $inv = Order::findOrFail($id);
                if (Auth::user()->id == $inv->vender_id || 'a' == Auth::user()->role_id) {
            ...
            if (Auth::user()->id == $inv->vender_id || 'a' == Auth::user()->role_id) {
                ...
                if ('shipped' == $request->status) {
                    ...
                    if ('1' == $this->config->sms_channel) {
                        ...
                        if ('msg91' == env('DEFAULT_SMS_CHANNEL') && '1' == $this->config->msg91_enable && '' != env('MSG91_AUTH_KEY')) {
                            try {
                ...
                } elseif ('processed' == $request->status) {
                } elseif ('pending' == $request->status) {
                } elseif ('delivered' == $request->status) {
                    ...
                    if (isset($aff_system) && 1 == $aff_system->enable_affilate && 1 == $aff_system->enable_purchase) {
                        $buyer = $inv->order->user;

                        if (1 == $buyer->purchaseorder()->count()) {
                            if (isset($buyer->onetimereferdata) && 0 == $buyer->onetimereferdata->procces) {
                                ...
                                if (isset($wallet->wallet) && 1 == $wallet->wallet->status) {
                    ...
                    if (1 == $wallet_system) {
                        ...
                        if (isset($inv->simple_product) && $inv->simple_product->cashback_settings && '' == $inv->cashback) {
                            ...
                            if (isset($wallet) && 1 == $wallet->status) {
                ...
                } elseif ('cancel_request' == $request->status) {
                    $newpendingpay = PendingPayout::where('orderid', '=', $inv->id)->first();
                    ...
                    // End
                } elseif ('canceled' == $request->status) {
                    $newpendingpay = PendingPayout::where('orderid', '=', $inv->id)->first();
                ...
                } elseif ('return_request' == $request->status) {
                    $newpendingpay = PendingPayout::where('orderid', '=', $inv->id)->first();
                    ...
                    // End
                } elseif ('returned' == $request->status) {
                    $newpendingpay = PendingPayout::where('orderid', '=', $inv->id)->first();
