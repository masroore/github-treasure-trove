...
class SellerPaymenyController extends Controller
{
    ...
    public function manualPayout(Request $request, $venderid, $orderid)
    {
        ...
        if ('Wallet' == $order->singleorder->order->payment_method) {
        ...
        if ($request->ajax()) {
            return Datatables::of($data)->addIndexColumn()
                ...
                ->addColumn('amount', function ($row) {
                    ...
                    return "<i class='cursym $defCurrency->currency_symbol'></i> $row->orderamount";
                })
                ->addColumn('detail', function ($row) {
                    $data = "<p><b>Seller:</b> $row->sellername<p>";
                    $data .= "<p><b>Store:</b> $row->storename</p>";
                ...
                ->addColumn('action', function ($row) {
                    return '<a href=' . route('seller.payout.show.complete', $row->id) . ' title="View payout detail" class="btn btn-primary-rgba btn-sm">View</a>';
                })
    ...
    public function show($id)
    {
        ...
        if (isset($order)) {
            ...
            if ($order->singleorder->variant) {
                ...
                if (0 == $order->singleorder->variant->products->return_avbl) {
                    return view('admin.seller.showdetail', compact('inv_cus', 'order'));
                }
                if (1 == $order->singleorder->variant->products->return_avbl) {
            ...
            if ($order->singleorder->simple_product) {
                if (0 == $order->singleorder->simple_product->return_avbl) {
                    return view('admin.seller.showdetail', compact('inv_cus', 'order'));
                }
                if (1 == $order->singleorder->simple_product->return_avbl) {
                    // find the order return policy and when its ended than show this page else throw an error
    ...
    public function showCompletePayout($id)
    {
        ...
        $inv_cus = Invoice::first();
        if ('Bank' == $payout->paidvia || 'manual' == $payout->txn_type) {
