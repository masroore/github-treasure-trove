...
class ReturnOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($orders as $key => $order) {
            if ('initiated' == $order->status && isset($order->getorder->order)) {
    ...
    public function paytouser(Request $request, $id)
    {
        ...
        if (Auth::check()) {
            if (isset($returnorder)) {
                if ('initiated' == $returnorder->status) {
                    if ('bank' == $returnorder->method_choosen) {
                        ...
                        if ('ret_ref' == $request->order_status) {
                        ...
                        if ('Wallet' == $returnorder->getorder->order->payment_method) {
                        ...
                        if ('Instamojo' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaInstamojo($request, $returnorder);
                        } elseif ('Paypal' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaPaypal($request, $returnorder);
                        } elseif ('Stripe' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaStripe($request, $returnorder);
                        } elseif ('PayU' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaPayU($request, $returnorder);
                        } elseif ('Razorpay' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaRazorPay($request, $returnorder);
                        } elseif ('Paytm' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaPaytm($request, $returnorder);
                        } elseif ('Braintree' == $returnorder->getorder->order->payment_method) {
                            $status = $this->payviaBraintree($request, $returnorder);
                        } elseif ('Paystack' == $returnorder->getorder->order->payment_method) {
                        ...
                        if (true == $status) {
                            try {
                                ...
                                if ('1' == $this->config->sms_channel) {
                                    ...
                                    if ('msg91' == env('DEFAULT_SMS_CHANNEL') && '1' == $this->config->msg91_enable) {
                                        try {
    ...
    public function payviaRazorPay($request, $returnorder)
    {
        ...
        try {
            ...
            $result = $refund->refund(['amount' => round($returnorder->amount * 100, 2)]);
