...
class InstallerController extends Controller
{
    ...
    public function eula()
    {
        if (0 == env('IS_INSTALLED')) {
            ...
            if ($getdraft) {
                ...
                if ('gotoserverpage' == $getdraft) {
                    return redirect()->route('servercheck');
                }

                if ('gotoverifypage' == $getdraft) {
                    return redirect()->route('verifyApp');
                }

                if ('gotostep1' == $getdraft) {
                    return redirect()->route('installApp');
                }

                if ('gotostep2' == $getdraft) {
                    return redirect()->route('get.step2');
                }

                if ('gotostep3' == $getdraft) {
                    return redirect()->route('get.step3');
                }

                if ('gotostep4' == $getdraft) {
                    return redirect()->route('get.step4');
                }

                if ('gotostep5' == $getdraft) {
    ...
    public function storeStep3(Request $request)
    {
        ...
        $currency->currencyextract()->updateOrCreate([
            ...
            'add_amount' => 0.00,
            'currency_symbol' => 'USD' == $currency->code ? 'fa fa-dollar' : $request->currency_symbol,
            'default_currency' => 1,
        ...
        if ($file = $request->file('logo')) {
            if (null != $$newGenral->logo && file_exists(public_path() . '/images/genral/' . $newGenral->logo)) {
        ...
        if ($file = $request->file('favicon')) {
            if (null != $newGenral->fevicon && file_exists(public_path() . '/images/genral/' . $newGenral->fevicon)) {
            ...
            $optimizeImage->resize(100, 100, function ($constraint): void {
                $constraint->aspectRatio();
            });
    ...
    public function storeStep5(Request $request)
    {
        ...
        if ($file = $request->file('storelogo')) {
            ...
            $optimizeImage->resize(300, 300, function ($constraint): void {
                $constraint->aspectRatio();
            });
