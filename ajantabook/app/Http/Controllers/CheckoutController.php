...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            foreach ($cart_table as $carts) {
                ...
                if (isset($cart->simple_product)) {
                    if (null == $cart->simple_product->max_order_qty) {
        ...
        if (Auth::check()) {
            ...
            foreach ($cart_table as $key => $cart) {
                if ($cart->product && 0 == $cart->product->free_shipping) {
                    ...
                    if (!empty($free_shipping)) {
                        if ('Shipping Price' == $free_shipping->name) {
                            ...
                            if (1 == $free_shipping->whole_order) {
            ...
            if (0 != $cartamountsetting->cart_amount && '' != $cartamountsetting->cart_amount) {
    ...
    public function orderReview()
    {
        ...
        $cart_table->each(function ($cart) use ($count): void {
            if ('localpickup' != $cart->ship_type && $cart->variant && $cart->product) {
                $cart->shipping = (float) ShippingPrice::calculateShipping($cart);
                $cart->save();
            }

            if ('localpickup' != $cart->ship_type && $cart->simple_product) {
            ...
            if (1 == get_default_shipping()->whole_order) {
                ...
                if ('localpickup' != $cart->ship_type && $cart->product && 0 == $cart->product->free_shipping) {
                    $count->push(1);
                }

                if ($cart->simple_product && 'localpickup' != $cart->ship_type && 0 == $cart->simple_product->free_shipping) {
                    $count->push(1);
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($file = $request->file('image')) {
            ...
            $optimizeImage->resize(200, 200, function ($constraint): void {
