...
class FullOrderCancelController extends Controller
{
    ...
    public function cancelOrder(Request $request, $id)
    {
        ...
        foreach ($order->invoices as $value) {
            if ('0' != $value->variant->products->cancel_avl) {
        ...
        if ($order->user_id == Auth::user()->id || 'a' == Auth::user()->role_id) {
            if ('COD' != $order->payment_method) {
                if ('bank' == $request->source) {
                    ...
                    if ('Wallet' == $order->payment_method) {
                        if (1 != $this->wallet_system) {
                        ...
                        if (isset($order->user->wallet) && 1 == $order->user->wallet->status) {
                            $cancelorderlog = new FullOrderCancelLog();
                            $cancelorderlog->order_id = $order->id;
                            $cancelorderlog->inv_id = $invArray;
                            $cancelorderlog->user_id = $order->user_id;
                    ...
                    } elseif ('PayPal' == $order->payment_method) {
                    ...
                    } elseif ('Stripe' == $order->payment_method) {
                        $stripe = new Stripe();
                    ...
                    } elseif ('Instamojo' == $order->payment_method) {
                    ...
                    } elseif ('PayU' == $order->payment_method) {
                        ...
                        return back();
                    } elseif ('Paytm' == $order->payment_method) {
                    ...
                    } elseif ('Razorpay' == $order->payment_method) {
                    ...
                    } elseif ('Braintree' == $order->payment_method) {
                    ...
                    } elseif ('Paystack' == $order->payment_method) {
            ...
            } elseif ('BankTransfer' == $order->payment_method) {
                if ('bank' == $request->source) {
                    $cancelorderlog = new FullOrderCancelLog();
                    $cancelorderlog->order_id = $order->id;
                    $cancelorderlog->inv_id = $invArray;
            ...
            if (1 == $status) {
                ...
                foreach ($order->invoices as $value) {
                    if (0 != $value
                        ->variant
                        ->products->cancel_avl) {
                        if ('COD' != $order->payment_method && 'bank' != $request->source) {
