...
class POSController extends Controller
{
    public function index(Request $request)
    {
        ...
        when($request->has('category_id') && 0 != $request['category_id'], function ($query) use ($request): void {
    ...
    public function quick_view(Request $request)
    {
        ...
        $products = DB::table('simple_products')->
        when($request->has('product_id') && 0 != $request['product_id'], function ($query) use ($request): void {
            $query->where('child_id', [[['child_id' => (string) $request['product_id']]]]);
        })
            ->when($request->has('sub_cat') && 0 != $request['sub_cat'], function ($query) use ($request): void {
    ...
    public function variant_price(Request $request)
    {
        ...
        return ['price' => ($price * $request->quantity) + $addon_price];
    }
    ...
    {
        if ('percent' == $request->type && $request->discount < 0) {
            Toastr::error(translate('Extra_discount_can_not_be_less_than_0_percent'));

            return back();
        }
        if ('percent' == $request->type && $request->discount > 100) {
    ...
    public function addToCart(Request $request)
    {
        // print_r($request);die;
        $products = DB::table('simple_products')->
        when($request->has('id') && 0 != $request['id'], function ($query) use ($request): void {
            $query->where('child_id', [[['child_id' => (string) $request['id']]]]);
        })
            ->when($request->has('subcategory') && 0 != $request['subcategory'], function ($query) use ($request): void {
        ...
        if ($request->has('single_p_id') && 0 != $request['single_p_id']) {
                    ...
                    if (\count($request->session()->get('cart1')) > 0) {
                        foreach ($request->session()->get('cart1') as $key => $cartItem) {
                            if (\is_array($cartItem) && $cartItem['id'] == $request['id'] && $cartItem['variant'] == $request['subcategory']) {
                                ...
                                $data_id[] = $remove_single_from_set;
    ...
    public function order_list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $query = $query->where(function ($q) use ($key): void {
                foreach ($key as $value) {
    ...
    public function place_order(Request $request)
    {
        ...
        $order->coupon = 0 == $request->coupon_discount_title ? null : 'coupon_discount_title';
        ...
        foreach ($cart as $c) {
            if (\is_array($c)) {
                ...
                $price = $c['price'];
                $parent_ids[$c['name']] = ['class_id' => $c['id'], 'school_id' => $c['subcategory'], 'quantity' => $c['quantity'], 'product_amt' => $product_subtotal, 'tax' => $c['tax']];
                foreach ($c['set_array_list'] as $key12 => $set_json) {
                    SimpleProduct::where('id', $set_json['single_product_id'])->update(['stock' => DB::raw('stock-1')]);

                    $simple_pro_idss[$c['id']][$set_json['single_product_id']] = ['class_name' => $c['name'], 'id' => $set_json['single_product_id'], 'name' => $set_json['name']];
                }
        ...
        if (isset($cart['extra_discount'])) {
            $extra_discount = 'percent' == $cart['extra_discount_type'] && $cart['extra_discount'] > 0 ? (($total_price * $cart['extra_discount']) / 100) : $cart['extra_discount'];
