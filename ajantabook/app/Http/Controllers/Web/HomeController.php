...
class HomeController extends Controller
{
    ...
    public function simple_products()
    {
        ...
        $simple_products = SimpleProduct::with('category')->whereHas('category', function ($q): void {
            $q->where('status', '=', '1');
        ...
        return $simple_products->map(function ($content) use ($conversion_rate) {
            ...
            $content['is_in_wishlist'] = false == inwishlist($content->id) ? 0 : 1;
    ...
    public function slider()
    {
        ...
        $sliders = $sliders->map(function ($query) {
            ...
            $content['moredescription'] = null != $query->moredesc ? $query->moredesc : 'Not found';
    ...
    public function featuredProducts()
    {
        ...
        $featuredproducts = Product::with('category')->whereHas('category', function ($q): void {
            $q->where('status', '=', '1');
        })->with('subcategory')->wherehas('subcategory', function ($q): void {
            $q->where('status', '1');
    ...
    public function topProducts()
    {
        ...
        if (isset($top_categories) && '' != $top_categories->category_ids) {
            foreach ($top_categories->category_ids as $category) {
                ...
                if (isset($category) && '1' == $category->status) {
                    $result[] = [
    ...
    public function categoryproducts($top_categories, $category)
    {
        ...
        $topcatproducts = Product::with('category')->whereHas('category', function ($q) use ($category): void {
        ...
        return $topcatproducts->filter()->shuffle();
    }
    ...
    public function getProducts($type)
    {
        ...
        if ('all' == $type) {
            $products = Product::with('category')->whereHas('category', function ($q): void {
                $q->where('status', '=', '1');
            })->with('subcategory')->wherehas('subcategory', function ($q): void {
                $q->where('status', '1');
            })->with('vender')->whereHas('vender', function ($query) use ($sellerSystem): void {
    ...
    public function tabbed_simple_products($type)
    {
        ...
        if ('all' == $type) {
            ...
            if ($category) {
                $simple_products = SimpleProduct::with('category')->whereHas('category', function ($q) use ($category): void {
    ...
    public function hotdeals()
    {
        ...
        if ($simple_products_hotdeals) {
            $simple_products_hotdeals = $simple_products_hotdeals->map(function ($sp) use ($conversion_rate, $content) {
                ...
                $content['thumbnail'] = url('images/simple_products/' . $sp->thumbnail);
                $content['hover_thumbnail'] = url('images/simple_products/' . $sp->hover_thumbnail);
                $content['is_in_wishlist'] = false == inwishlist($sp->id) ? 0 : 1;
                $content['stock'] = $sp->stock;
                $content['featured'] = $sp->featured;
                $content['type'] = $sp->type;
                $content['rating'] = 0;
    ...
    public function topmenus()
    {
        ...
        $menus = $menus->map(function ($item) {
            if ('1' == $item->show_cat_in_dropdown && null != $item->linked_parent) {
                $item['megamenu'] = $this->megamenu($item)->getData();

                return $item;
            }
            if ('1' == $item->show_child_in_dropdown && null != $item->linked_parent) {
    ...
    {
        if (1 == $menu->show_cat_in_dropdown && null != $menu->linked_parent) {
            $result = [];
    ...
    {
        if (1 == $menu->show_child_in_dropdown && null != $menu->linked_parent) {
            $result = [];
    ...
    public function wishlistcount()
    {
        try {
            if (Auth::check()) {
                ...
                $data->map(function ($q) {
                    if (isset($q->variant->products, $q->variant)) {
                        if ('1' == $q->variant->products->status) {
                            return $q;
                        }
                    }
                });
    ...
    public function comparecount()
    {
        ...
        return response()->json(null != Session::get('comparison') ? \count(Session::get('comparison')) : 0);
    }
