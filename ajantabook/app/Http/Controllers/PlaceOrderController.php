...
class PlaceOrderController extends Controller
{
    public function placeorder($txn_id, $payment_method, $order_id, $payment_status, $saleid = null, $file = null)
    {
        ...
        foreach ($cart_table as $key => $cart) {
            $venderarray[] = $cart->vender_id;

            $qty_total = $qty_total + $cart->qty;

            if ('' != $cart->pro_id) {
                $pro_id[] = $cart->variant_id;
                $mainproid[] = $cart->pro_id;
            }

            if ('' != $cart->simple_pro_id) {
                $simpleproid[] = $cart->simple_pro_id;
            }
        ...
        foreach ($cart_table2 as $key => $invcart) {
            ...
            if ($invcart->product && 'v' == $invcart->product->vender->role_id) {
                $newInvoice->paid_to_seller = 'NO';
            }

            if ($invcart->simple_product && 'v' == $invcart->simple_product->store->user->role_id) {
                $newInvoice->paid_to_seller = 'NO';
            }

            if (isset($invcart->simple_product)) {
                if (1 == $invcart->simple_product->pre_order && $invcart->simple_product->product_avbl_date > date('Y-m-d h:i:s')) {
                    $newInvoice->type = __('pre_order');

                    if ('partial' == $invcart->simple_product->preorder_type) {
                        $productprice = 0 != $invcart->simple_product->offer_price ? $invcart->simple_product->offer_price : $invcart->simple_product->price;
                        ...
                        $rem_price = 0 != $invcart->semi_total ? $invcart->semi_total - $invcart->tax_amount : $invcart->price_total - $invcart->tax_amount;
        ...
        if ('1' == Cart::isCoupanApplied() && Cart::getCoupanDetail()) {
            // Coupon applied //
