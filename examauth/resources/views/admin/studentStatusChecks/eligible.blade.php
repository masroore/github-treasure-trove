...
<div class="card">
    <div class="card-header">
        Eligible Students For <strong><span style="color: rgb(17, 90, 116)">{{ $course->course_code }}</span></strong> Examination, Session {{ date('Y') }}/{{ date('Y') + 1 }}    </div>
                ...
                <tbody>
                    @foreach($studentProfiles as $key => $studentProfile)
                        <tr data-entry-id="{{ $studentProfile->id }}">
                            ...
                            <td>
                                <span style="background-color: rgb(9, 99, 109); color:#FFF; padding:3px; border-radius: 4px">{{ $studentProfile->confirm ?? 'Not Confirmed' }}</span>
                            </td>
                            <td>
                                ...
                                @can('student_profile_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.eligible', $studentProfile->id) }}&id={{ $studentProfile->id }}">
                                        Confirm
                                    </a>
                                @endcan
