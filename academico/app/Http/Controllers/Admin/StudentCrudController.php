    ...
    public function setupListOperation(): void
    {
        ...
        CRUD::setColumns([
            ...
            [
                ...
                'searchLogic' => function ($query, $column, $searchTerm): void {
                    $query->orWhereHas('user', function ($q) use ($searchTerm): void {
        ...
        CRUD::addFilter(
            ...
            $this->crud->query = $this->crud->query->whereHas('enrollments', function ($query) use ($value) {
                return $query->whereHas('course', function ($q) use ($value): void {
                    $q->where('period_id', $value);
                });
            });
        },
            function (): void { // if the filter is NOT active (the GET parameter "checkbox" does not exit)
          }
        );
        ...
        CRUD::addFilter([ // select2_multiple filter
            ...
            return Period::all()->pluck('name', 'id')->toArray();
        }, function ($values): void { // if the filter is active
