...
class Customer extends Authenticatable
{
    ...
    public function priceFormat($price)
    {
        ...
        return (('pre' == $settings['site_currency_symbol_position']) ? $settings['site_currency_symbol'] : '') . number_format($price, Utility::getValByName('decimal_number')) . (('post' == $settings['site_currency_symbol_position']) ? $settings['site_currency_symbol'] : '');
    }
    ...
    public function invoiceChartData()
    {
        ...
        $progressData['unpaidPr'] = (0 != $totalInvoice) ? ($totalUnpaidInvoice * 100) / $totalInvoice : 0;
        $progressData['paidPr'] = (0 != $totalInvoice) ? ($totalPaidInvoice * 100) / $totalInvoice : 0;
        $progressData['partialPr'] = (0 != $totalInvoice) ? ($totalPartialInvoice * 100) / $totalInvoice : 0;
        $progressData['duePr'] = (0 != $totalInvoice) ? ($totalDueInvoice * 100) / $totalInvoice : 0;
    ...
    {
        return Proposal::where('customer_id', $customerId)->orderBy('issue_date', 'desc')->get();
    }
    ...
    {
        return Invoice::where('customer_id', $customerId)->count();
    }
}
