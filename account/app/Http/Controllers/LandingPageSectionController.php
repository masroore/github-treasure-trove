...
class LandingPageSectionController extends Controller
{
    ...
    public function setConetent(Request $request)
    {
        if ('super admin' == Auth::user()->type) {
            ...
            if (null !== $get_section) {
                if ('section-plan' == $get_section->section_type) {
                    $content = $get_section->default_content;
                } else {
                    $data = [];
                    if ('' == $get_section->content || null == $get_section->content) {
                    ...
                    foreach ($decode_content as $key => $value) {
                        ...
                        if ('logo' == $key) {
                            ...
                            if ('new_tab' == $content_type) {
                                $sys_data['id'] = \count($value) + 1;
                                $sys_data['name'] = $system_new_tab;
                                $sys_data['data'] = [];
                                $value[] = $sys_data;
                                $data['system'] = $value;
                            } elseif ('update_tab_content' == $content_type) {
                                $system_data = [];
                            ...
                            } elseif ('new_tab_content' == $content_type) {
                                $system_inner_data = [];
                                foreach ($value as $key => $sys_value) {
                                    if ($sys_value->id == $system_element_id) {
                                        $no = 1;
                            ...
                            } elseif ('remove_element' == $content_type) {
                                foreach ($value as $key => $sys_value) {
                                    if ($sys_value->id == $system_element_id) {
                                    } else {
                                        $system_data[] = $sys_value;
                            ...
                            } elseif ('remove_element_data' == $content_type) {
                                $system_data = [];
                                foreach ($value as $key => $sys_value) {
                                    $system_inner_data = [];
                                    if ($sys_value->id == $system_element_id) {
                        ...
                        } elseif ('testimonials' == $key) {
                            $testinomial_data = [];
                            if ('update_section' == $content_type) {
                            ...
                            } elseif ('new_section' == $content_type) {
                                $no = 1;
                                $data_text = [];
    ...
    public function show(Request $request, $id)
    {
        ...
        if (null !== $get_content) {
            ...
            if ('' == $get_content->content || null == $get_content->content) {
